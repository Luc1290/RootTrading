...
-- VERSION CORRIGÉE - Colonnes et contraintes validées
...
-- Index pour améliorer les requêtes de réservation (utilise reservation_id, pas reference_id)
...
-- Mise à jour des contraintes pour inclure le type 'deposit'
...
CHECK (transaction_type::text = ANY (ARRAY[
    'reserve'::character varying, 
    'release'::character varying, 
    'allocate'::character varying, 
    'reallocate'::character varying, 
...
FROM pg_constraint 
WHERE conrelid = 'pocket_transactions'::regclass;