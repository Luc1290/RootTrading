...
class VolumeContextAnalyzer:
    ...
    def __init__(
        ...
        self.context_configs = {
            VolumeContextType.DEEP_OVERSOLD: {
                "min_ratio": 0.8,
                "ideal_ratio": 1.8,
                "description": "Oversold extrême - tolérance élevée",
            },
            VolumeContextType.MODERATE_OVERSOLD: {
                "min_ratio": 1.0,
                "ideal_ratio": 2.0,
                "description": "Oversold modéré - tolérance moyenne",
            },
            VolumeContextType.OVERSOLD_BOUNCE: {
                "min_ratio": 1.2,
                "ideal_ratio": 2.2,
                "description": "Bounce oversold - volume important",
            },
            VolumeContextType.LOW_VOLATILITY: {
                "min_ratio": 0.9,
                "ideal_ratio": 1.6,
                "description": "Faible volatilité - seuils réduits",
            },
            VolumeContextType.HIGH_VOLATILITY: {
                "min_ratio": 1.8,
                "ideal_ratio": 3.0,
                "description": "Forte volatilité - seuils élevés",
            },
            VolumeContextType.CONSOLIDATION_BREAK: {
                "min_ratio": 1.4,
                "ideal_ratio": 2.5,
                "description": "Sortie consolidation - confirmation volume",
            },
            VolumeContextType.BREAKOUT: {
                "min_ratio": 2.0,
                "ideal_ratio": 4.0,
                "description": "Breakout - volume critique",
            },
            VolumeContextType.TREND_CONTINUATION: {
                "min_ratio": 1.3,
                "ideal_ratio": 2.3,
                "description": "Continuation tendance - volume normal",
            },
            VolumeContextType.PUMP_START: {
                "min_ratio": 2.5,
                "ideal_ratio": 5.0,
                "description": "Début pump - volume massif requis",
            },
            VolumeContextType.REVERSAL_PATTERN: {
                "min_ratio": 1.6,
                "ideal_ratio": 2.8,
                "description": "Pattern reversal - volume confirmateur",
            },
            VolumeContextType.NEUTRAL: {
                "min_ratio": 1.5,
                "ideal_ratio": 2.5,
                "description": "Contexte neutre - seuils standards",
            },
        }
    ...
    def _detect_market_context(
        ...
        if len(volumes) >= 5:
            ...
            volume_increase = (
                volumes[-1] / mean_volume
                if mean_volume > 0 and not np.isnan(mean_volume)
                else 1.0
            )
        else:
            volume_increase = 1.0
    ...
    def _generate_recommendations(
        ...
        if spike_detected:
            recommendations.append(
                "🚀 Volume spike détecté - Action immédiate possible"
            )
        ...
        if context.context_type in [
            ...
            recommendations.append(
                "💡 Conditions oversold - Tolérance volume appliquée"
            )

        if context.context_type == VolumeContextType.BREAKOUT:
            recommendations.append(
                "⚡ Contexte breakout - Volume critique pour confirmation"
            )
    ...
    def get_adaptive_threshold(
        ...
        """
        analysis = self.analyze_volume_context(
            volumes, closes, symbol=symbol, signal_type=signal_type
        )
