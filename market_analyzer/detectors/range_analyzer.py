...
class RangeAnalyzer:
    ...
    def detect_breakout(
        ...
        try:
            ...
            breakout_down = current_low < range_info.range_low * (
                1 - self.breakout_threshold
            )
    ...
    def _identify_range_boundaries(
        ...
        try:
            ...
            if resistance_candidates:
                ...
                resistance_groups = self._group_similar_levels(
            ...
            if support_candidates:
                ...
                support_groups = self._group_similar_levels(
    ...
    def _calculate_position_in_range(
    ...
    def _estimate_range_duration(
        self, closes: np.ndarray, range_high: float, range_low: float
    ) -> int:
    ...
    def _calculate_range_efficiency(
        self, closes: np.ndarray, range_high: float, range_low: float
    ) -> float:
    ...
    def _calculate_boundary_volume(
        self,
        highs: np.ndarray,
        lows: np.ndarray,
        volumes: np.ndarray,
    ...
    def _determine_preferred_direction(
        self, closes: np.ndarray, volumes: np.ndarray, current_position: float
    ) -> str:
    ...
    def _analyze_bearish_breakout(
        ...
        price_movement = (
            (range_info.range_low - current_price) / range_info.range_low * 100
        )
    ...
    def _check_momentum_confirmation(
        self, closes: np.ndarray, is_bullish: bool
    ) -> bool:
