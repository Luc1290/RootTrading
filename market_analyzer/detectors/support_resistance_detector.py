...
class PriceLevel:
    ...
    slope: float | None = None  # Pour trendlines
    timeframe: str = "5m"
...
class SupportResistanceDetector:
    ...
    def detect_levels(
        ...
        try:
            ...
            volume_levels = self._detect_volume_levels(
            ...
            # 4. Lignes de tendance
            trendline_levels = self._detect_trendlines(
                highs, lows, closes, current_price, timeframe
            )
            ...
            final_levels = self._calculate_final_strength(
                consolidated_levels, highs, lows, closes, volumes
            )
            ...
            logger.exception("Erreur détection niveaux S/R")
            return []
            ...
            for peak_idx in high_peaks[-10:]:  # Derniers 10 pics
                if peak_idx < len(highs):
                    ...
                    touches = self._count_touches(
                        price, highs, lows, self.price_tolerance
                    )
            ...
            if va_high_idx != poc_idx:
                ...
                levels.append(
                    PriceLevel(
                        ...
                        touches=1,
                        volume_strength=float(
                            volume_profile[va_high_idx] / total_volume
                        ),
                        distance_from_price=abs(vah_price - current_price)
                        / current_price,
                        last_test_age=0,
            ...
            if va_low_idx != poc_idx:
                ...
                levels.append(
                    PriceLevel(
                        ...
                        touches=1,
                        volume_strength=float(
                            volume_profile[va_low_idx] / total_volume
                        ),
                        distance_from_price=abs(val_price - current_price)
                        / current_price,
                        last_test_age=0,
                        break_probability=0.4,
                        timeframe=timeframe,
    ...
    def _detect_psychological_levels(
        self, closes: np.ndarray, current_price: float, timeframe: str
    ) -> list[PriceLevel]:
            ...
            for step in chosen_steps:
                ...
                while level <= max_level:
                    if (
                        ...
                        and abs(level - current_price) / current_price > 0.005
                    ):
                        # Vérifier si le niveau a de l'historique
                        touches = self._count_psychological_touches(
            ...
            if support_line is not None:
                ...
                levels.append(
                    PriceLevel(
                        ...
                        volume_strength=0.3,
                        distance_from_price=abs(current_support - current_price)
                        / current_price,
            ...
            if resistance_line is not None:
                ...
                levels.append(
                    PriceLevel(
                        ...
                        volume_strength=0.3,
                        distance_from_price=abs(current_resistance - current_price)
                        / current_price,
    ...
    def _consolidate_levels(
        ...
        while i < len(levels):
            ...
            while j < len(levels):
                if (
                    abs(levels[j].price - level.price) / level.price
    ...
    def _merge_levels(self, levels: list[PriceLevel]) -> PriceLevel:
        ...
        if total_confidence > 0:
            weighted_price = (
                sum(level.price * level.confidence for level in levels)
                / total_confidence
            )
        else:
        ...
        return PriceLevel(
            ...
            break_probability=float(
                np.mean([level.break_probability for level in levels])
            ),
    ...
    def _calculate_final_strength(
        ...
        for level in levels:
            ...
            level.touches = self._count_touches(
                level.price, highs, lows, self.price_tolerance
            )
            ...
            level.confidence = min(
                level.confidence + touch_bonus + volume_bonus - age_penalty, 95
            )
    ...
    def _count_psychological_touches(
        self, level: float, closes: np.ndarray, tolerance: float
    ) -> int:
    ...
    def _calculate_break_probability(
        self, level: float, closes: np.ndarray, is_resistance: bool
    ) -> float:
            ...
            if recent_high > current_price:
                levels.append(
                    PriceLevel(
                        ...
                        touches=1,
                        volume_strength=0.0,
                        distance_from_price=abs(recent_high - current_price)
                        / current_price,
                        last_test_age=0,
            ...
            if recent_low < current_price:
                levels.append(
                    PriceLevel(
                        ...
                        touches=1,
                        volume_strength=0.0,
                        distance_from_price=abs(recent_low - current_price)
                        / current_price,
                        last_test_age=0,
                        break_probability=0.5,
                        timeframe=timeframe,
                ...
                if day_high > current_price and day_high != recent_high:
                    levels.append(
                        PriceLevel(
                            ...
                            touches=1,
                            volume_strength=0.0,
                            distance_from_price=abs(day_high - current_price)
                            / current_price,
                            last_test_age=0,
                            break_probability=0.5,
                            timeframe=timeframe,
                ...
                if day_low < current_price and day_low != recent_low:
                    levels.append(
                        PriceLevel(
                            ...
                            touches=1,
                            volume_strength=0.0,
                            distance_from_price=abs(day_low - current_price)
                            / current_price,
                            last_test_age=0,
                            break_probability=0.5,
                            timeframe=timeframe,
    ...
    def get_key_levels_summary(
        ...
        supports = [
            ...
            and level.level_type in [LevelType.SUPPORT, LevelType.TRENDLINE]
        ]
        resistances = [
            level
            for level in levels
            if level.price > current_price
            and level.level_type in [LevelType.RESISTANCE, LevelType.TRENDLINE]
        ]
        ...
        return {
            ...
            "strong_supports": [
                s.to_dict()
                for s in supports
                if s.strength in [LevelStrength.STRONG, LevelStrength.MAJOR]
            ][:3],
            "strong_resistances": [
                r.to_dict()
                for r in resistances
                if r.strength in [LevelStrength.STRONG, LevelStrength.MAJOR]
            ][:3],
