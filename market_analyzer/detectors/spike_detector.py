...
class SpikeDetector:
    ...
    def detect_spikes(self,
                     ...
                     timestamps: Optional[List[str]] = None) -> List[SpikeEvent]:
        """
        ...
        try:
            ...
            combined_spikes = self._detect_combined_spikes(highs, lows, closes, volumes, timestamps)
            all_spikes.extend(combined_spikes)
            
            # 4. Détecter les patterns de manipulation
            manipulation_spikes = self._detect_manipulation_patterns(highs, lows, closes, volumes, timestamps)
            all_spikes.extend(manipulation_spikes)
            
            # 5. Détecter les liquidation sweeps
            liquidity_spikes = self._detect_liquidity_sweeps(highs, lows, closes, volumes, timestamps)
    ...
    def _detect_price_spikes(self, highs: np.ndarray, lows: np.ndarray,
                           closes: np.ndarray, timestamps: List[str]) -> List[SpikeEvent]:
        ...
        try:
            ...
            for i in range(self.lookback_window, len(price_changes)):
                ...
                if abs(z_score) > self.price_z_threshold:
                    spike_type = SpikeType.PRICE_SPIKE_UP if current_change > 0 else SpikeType.PRICE_SPIKE_DOWN
                    ...
                    # Impact potentiel
                    impact = self._predict_price_impact(current_change, pre_trend, z_score)
                    
                    spikes.append(SpikeEvent(
                        timestamp=timestamps[i + 1] if i + 1 < len(timestamps) else f"bar_{i+1}",
    ...
    def _detect_volume_spikes(self, volumes: np.ndarray, closes: np.ndarray,
                            timestamps: List[str]) -> List[SpikeEvent]:
    ...
    def _detect_combined_spikes(self, highs: np.ndarray, lows: np.ndarray,
        ...
        try:
            ...
            for i in range(self.lookback_window, len(closes) - 1):
                ...
                volume_significant = volume_z > self.volume_z_threshold and volume_ratio > 2.0
                ...
                if price_significant and volume_significant:
                    ...
                    # Intensité combinée
                    intensity = self._calculate_combined_intensity(abs(price_z), volume_z, volume_ratio)
                    
                    # Impact probablement fort avec prix + volume
                    impact = self._predict_combined_impact(current_price_change, volume_ratio, price_z)
    ...
    def _detect_manipulation_patterns(self, highs: np.ndarray, lows: np.ndarray,
                                    closes: np.ndarray, volumes: np.ndarray,
                                    timestamps: List[str]) -> List[SpikeEvent]:
        ...
        try:
            for i in range(10, len(closes) - 5):  # Besoin de contexte avant/après
                ...
                pump_pattern = self._detect_pump_pattern(window_closes, window_volumes, i-10)
                if pump_pattern:
                    spikes.append(SpikeEvent(
                        timestamp=timestamps[i] if i < len(timestamps) else f"bar_{i}",
                ...
                dump_pattern = self._detect_dump_pattern(window_closes, window_volumes, i-10)
                if dump_pattern:
                    spikes.append(SpikeEvent(
                        timestamp=timestamps[i] if i < len(timestamps) else f"bar_{i}",
    ...
    def _detect_liquidity_sweeps(self, highs: np.ndarray, lows: np.ndarray,
                               closes: np.ndarray, volumes: np.ndarray,
                               timestamps: List[str]) -> List[SpikeEvent]:
        ...
        try:
            for i in range(20, len(closes)):
                ...
                avg_volume = np.mean(volumes[i-10:i]) if i >= 10 else np.mean(volumes[:i])
                volume_ratio = current_volume / avg_volume if avg_volume > 0 else 1
                
                # Sweep au-dessus de la résistance
                if (current_high > resistance_level * 1.001 and  # Dépassement de 0.1%
                    closes[i] < resistance_level and  # Mais fermeture en dessous
                    volume_ratio > 1.5):  # Avec volume élevé
                ...
                elif (current_low < support_level * 0.999 and  # Cassure de 0.1%
                      closes[i] > support_level and  # Mais fermeture au-dessus
                      volume_ratio > 1.5):  # Avec volume élevé
    ...
    def _calculate_volume_intensity(self, volume_ratio: float, z_score: float) -> SpikeIntensity:
    ...
    def _calculate_combined_intensity(self, price_z: float, volume_z: float, volume_ratio: float) -> SpikeIntensity:
    ...
    def _predict_price_impact(self, price_change: float, pre_trend: str, z_score: float) -> SpikeImpact:
        """Prédit l'impact du spike prix."""
        if abs(z_score) > 3.0:
            if (price_change > 0 and pre_trend == 'bearish') or (price_change < 0 and pre_trend == 'bullish'):
    ...
    def _predict_volume_impact(self, volume_ratio: float, price_change: float) -> SpikeImpact:
        """Prédit l'impact du spike volume."""
        if volume_ratio > 5 and abs(price_change) > 0.02:  # Volume énorme + mouvement prix
            return SpikeImpact.BREAKOUT
        elif volume_ratio > 3:
            return SpikeImpact.CONTINUATION
        else:
    ...
    def _predict_combined_impact(self, price_change: float, volume_ratio: float, price_z: float) -> SpikeImpact:
    ...
    def _classify_combined_spike(self, price_change: float, volume_ratio: float,
                               high: float, low: float, close: float) -> SpikeType:
    ...
    def _detect_pump_pattern(self, closes: np.ndarray, volumes: np.ndarray, offset: int) -> Optional[Dict]:
        ...
        if pump_rise > 0.05 and pump_fall < -0.03 and volume_ratio > 3:  # 5% montée, 3% chute, 3x volume
            return {
    ...
    def _detect_dump_pattern(self, closes: np.ndarray, volumes: np.ndarray, offset: int) -> Optional[Dict]:
        ...
        recovery = (closes[-1] - closes[min_idx]) / closes[min_idx] if closes[min_idx] != 0 else 0
    ...
    def _merge_spikes(self, spikes: List[SpikeEvent]) -> SpikeEvent:
        ...
        combined_confidence = min(sum(s.confidence for s in spikes) / len(spikes) + 10, 95)
    ...
    def _analyze_post_spike_impact(self, spikes: List[SpikeEvent], closes: np.ndarray) -> List[SpikeEvent]:
    ...
    def get_recent_spikes(self, spikes: List[SpikeEvent], max_age: int = 10) -> List[SpikeEvent]:
    ...
    def get_spike_summary(self, spikes: List[SpikeEvent]) -> Dict:
        ...
        return {
            ...
            'highest_confidence': max(spikes, key=lambda x: x.confidence).to_dict() if spikes else None
        }