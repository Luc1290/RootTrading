...
class MultiTimeframeAnalyzer:
    ...
    def __init__(self,
        ...
        if timeframes is None:
            timeframes = [TimeframeType.M5, TimeframeType.M15, TimeframeType.H1, TimeframeType.H4]
    ...
    def analyze_multiple_timeframes(self,
                                  ...
                                  enable_cache: bool = True) -> MultiTimeframeAnalysis:
        """
        ...
        try:
            ...
            for tf_str, data in timeframe_data.items():
                try:
                    ...
                    if tf in self.timeframes:
                        signal = self._analyze_single_timeframe(tf, data, current_price, symbol, enable_cache)
            ...
            key_support_levels = self._find_confluent_support_levels(timeframe_signals, current_price)
            key_resistance_levels = self._find_confluent_resistance_levels(timeframe_signals, current_price)
            ...
            entry_zones = self._identify_entry_zones(timeframe_signals, current_price, primary_trend)
            exit_zones = self._identify_exit_zones(timeframe_signals, current_price, primary_trend)
            
            # 8. Détecter les divergences
            divergence_alerts = self._detect_divergences(timeframe_signals)
            
            # 9. Évaluer le risque
            risk_level = self._assess_risk_level(trend_alignment, signal_strength, divergence_alerts)
            
            # 10. Calculer la confiance globale
            confidence = self._calculate_overall_confidence(timeframe_signals, trend_alignment)
    ...
    def _analyze_single_timeframe(self, timeframe: TimeframeType, 
                                 data: Dict, current_price: float,
                                 symbol: Optional[str] = None, enable_cache: bool = True) -> TimeframeSignal:
        ...
        try:
            ...
            regime = self.regime_detector.detect_regime(highs, lows, closes, volumes, symbol, True, enable_cache)
            ...
            support_levels = [l.price for l in sr_levels 
                            if l.price < current_price and 'support' in l.level_type.value]
            resistance_levels = [l.price for l in sr_levels 
                               if l.price > current_price and 'resistance' in l.level_type.value]
            
            # Analyse de tendance simple (with caching if symbol provided)
            trend_direction, trend_strength = self._analyze_trend_simple(closes, symbol, enable_cache)
            momentum_direction, momentum_strength = self._analyze_momentum_simple(closes, symbol, enable_cache)
    ...
    def _analyze_trend_simple(self, closes: np.ndarray, symbol: Optional[str] = None, enable_cache: bool = True) -> Tuple[str, float]:
        """Analyse simple de tendance."""
        if len(closes) < 20:
            return "neutral", 0.0
        
        from ..indicators.trend.moving_averages import calculate_ema_series, calculate_ema
    ...
    def _analyze_momentum_simple(self, closes: np.ndarray, symbol: Optional[str] = None, enable_cache: bool = True) -> Tuple[str, float]:
    ...
    def _analyze_trend_alignment(self, signals: Dict[str, TimeframeSignal]) -> TrendAlignment:
        """Analyse l'alignement des tendances entre timeframes."""
        if not signals:
            return TrendAlignment.MIXED
        
        bullish_count = sum(1 for s in signals.values() if s.trend_direction == "bullish")
        bearish_count = sum(1 for s in signals.values() if s.trend_direction == "bearish")
    ...
    def _calculate_overall_signal_strength(self, signals: Dict[str, TimeframeSignal]) -> SignalStrength:
        ...
        for tf_str, signal in signals.items():
            ...
            weighted_strength += float(signal.trend_strength) * weight * (signal.confidence / 100)
    ...
    def _find_confluent_support_levels(self, signals: Dict[str, TimeframeSignal], 
    ...
    def _find_confluent_resistance_levels(self, signals: Dict[str, TimeframeSignal], 
                                         current_price: float) -> List[float]:
    ...
    def _calculate_momentum_consensus(self, signals: Dict[str, TimeframeSignal]) -> str:
        ...
        bullish_count = sum(1 for s in signals.values() if s.momentum_direction == "bullish")
        bearish_count = sum(1 for s in signals.values() if s.momentum_direction == "bearish")
    ...
    def _identify_entry_zones(self, signals: Dict[str, TimeframeSignal], 
                             current_price: float, primary_trend: str) -> List[Dict]:
    ...
    def _identify_exit_zones(self, signals: Dict[str, TimeframeSignal], 
                            current_price: float, primary_trend: str) -> List[Dict]:
                ...
                for support in sorted(all_supports, reverse=True)[:3]:
                    if support < current_price:
                        exit_zones.append({
                            ...
                            'zone_high': support * 1.005,
                            'distance_pct': abs(support - current_price) / current_price * 100,
                            'profit_potential': (current_price - support) / current_price * 100
                        })
    ...
    def _detect_divergences(self, signals: Dict[str, TimeframeSignal]) -> List[str]:
        ...
        if "bullish" in trends and "bearish" in trends:
            ...
            if abs(bullish_count - bearish_count) <= 1:
                divergences.append("Divergence majeure: Tendances contradictoires entre timeframes")
        
        # Momentum vs trend
        for tf, signal in signals.items():
            if signal.trend_direction != signal.momentum_direction and signal.momentum_direction != "neutral":
                divergences.append(f"Divergence {tf}: Tendance {signal.trend_direction} vs Momentum {signal.momentum_direction}")
        
        return divergences
    
    def _assess_risk_level(self, trend_alignment: TrendAlignment, 
                          signal_strength: SignalStrength, divergences: List[str]) -> str:
        """Évalue le niveau de risque."""
        risk_score = 0
        
        # Alignement des tendances
        if trend_alignment in [TrendAlignment.FULLY_ALIGNED_BULL, TrendAlignment.FULLY_ALIGNED_BEAR]:
            risk_score += 0  # Faible risque
        elif trend_alignment in [TrendAlignment.MOSTLY_ALIGNED_BULL, TrendAlignment.MOSTLY_ALIGNED_BEAR]:
    ...
    def _calculate_overall_confidence(self, signals: Dict[str, TimeframeSignal], 
                                     trend_alignment: TrendAlignment) -> float:
    ...
    def _identify_next_key_levels(self, all_levels: List[float], current_price: float) -> List[float]:
    ...
    def _calculate_confluence_score(self,
        ...
        strength_bonus = {
            "very_weak": 0, "weak": 10, "moderate": 20,
            "strong": 30, "very_strong": 40
        }.get(signal_strength.value, 0)
        
        # Bonus selon l'alignement
        alignment_bonus = {
            "fully_aligned_bull": 30, "fully_aligned_bear": 30,
            "mostly_aligned_bull": 20, "mostly_aligned_bear": 20,
            "mixed": 5, "conflicting": -10
    ...
    def get_trading_recommendation(self, analysis: MultiTimeframeAnalysis, 
        ...
        if (analysis.signal_strength in [SignalStrength.STRONG, SignalStrength.VERY_STRONG] and
            analysis.trend_alignment in [TrendAlignment.FULLY_ALIGNED_BULL, TrendAlignment.MOSTLY_ALIGNED_BULL] and
            ...
            if isinstance(reasoning_list, list):
                ...
                recommendation['reasoning'] = ["Forte confluence haussière multi-timeframe"]
            
            # Prix d'entrée (niveau de support proche ou prix actuel)
            if analysis.key_support_levels:
                nearest_support = max([s for s in analysis.key_support_levels if s < current_price * 1.02])
            ...
            if analysis.key_support_levels:
                ...
                try:
                    entry_price_float = float(entry_price) if isinstance(entry_price, (int, float, str)) else current_price
                    supports_below = [s for s in analysis.key_support_levels if s < entry_price_float]
                    if supports_below:
                        recommendation['stop_loss'] = max(supports_below) * 0.995
                except (TypeError, ValueError):
                    # Si la conversion échoue, utiliser current_price
                    supports_below = [s for s in analysis.key_support_levels if s < current_price]
                    if supports_below:
                        recommendation['stop_loss'] = max(supports_below) * 0.995
        
        elif (analysis.signal_strength in [SignalStrength.STRONG, SignalStrength.VERY_STRONG] and
              analysis.trend_alignment in [TrendAlignment.FULLY_ALIGNED_BEAR, TrendAlignment.MOSTLY_ALIGNED_BEAR] and
              analysis.confidence > 70):
            ...
            if isinstance(reasoning_list, list):
                ...
                recommendation['reasoning'] = ["Forte confluence baissière multi-timeframe"]
            
            # Prix d'entrée (résistance proche ou prix actuel)
            if analysis.key_resistance_levels:
                nearest_resistance = min([r for r in analysis.key_resistance_levels if r > current_price * 0.98])
            ...
            if analysis.key_resistance_levels:
                ...
                try:
                    entry_price_float = float(entry_price) if isinstance(entry_price, (int, float, str)) else current_price
                    resistances_above = [r for r in analysis.key_resistance_levels if r > entry_price_float]
                    if resistances_above:
                        recommendation['stop_loss'] = min(resistances_above) * 1.005
                except (TypeError, ValueError):
                    # Si la conversion échoue, utiliser current_price
                    resistances_above = [r for r in analysis.key_resistance_levels if r > current_price]
                    if resistances_above:
                        recommendation['stop_loss'] = min(resistances_above) * 1.005
