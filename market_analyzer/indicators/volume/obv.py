...
def obv_signal(
    ...
    obv_direction = (
        ...
        else "down" if current_obv < previous_obv else "flat"
    )
    price_direction = (
        "up"
        if current_price > previous_price
        else "down" if current_price < previous_price else "flat"
    )
...
def detect_obv_divergence(
    ...
    if len(price_lows) >= 2 and len(obv_lows) >= 2:
        ...
        if (
            ...
            and obv_val1 is not None
            and obv_val2 is not None
            and price_val1 < price_val2
            and obv_val1 > obv_val2
        ):
            return "bullish_divergence"
...
def obv_trend_strength(obv_values: list[float | None], period: int = 10) -> str:
    """
...
def _calculate_obv_manual(prices: np.ndarray, volumes: np.ndarray) -> float | None:
