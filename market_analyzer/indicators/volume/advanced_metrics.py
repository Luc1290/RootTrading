...
def calculate_quote_volume_ratio(quote_volumes: List[float], 
...
def calculate_avg_trade_size(volume: float, 
                           number_of_trades: int) -> Optional[float]:
    """
...
def calculate_trade_intensity(trades_counts: List[int], 
                            lookback: int = 20) -> Optional[float]:
    """
...
def analyze_volume_quality(volumes: List[float],
    ...
    # Calculs de base
    if quote_volumes:
        result['quote_volume_ratio'] = calculate_quote_volume_ratio(quote_volumes, lookback)
    
    if volumes and trades_counts and len(volumes) > 0 and len(trades_counts) > 0:
        current_volume = volumes[-1]
        current_trades = trades_counts[-1]
        
        result['avg_trade_size'] = calculate_avg_trade_size(current_volume, current_trades)
...
def detect_volume_anomaly(avg_trade_sizes: List[float],
                        trades_counts: List[int],
    ...
    if avg_size_std > 0 and (current_avg_size - avg_size_mean) / avg_size_std > threshold:
        if trades_std > 0 and (current_trades - trades_mean) / trades_std < -threshold/2:
    ...
    if trades_std > 0 and (current_trades - trades_mean) / trades_std > threshold:
        if avg_size_std > 0 and (current_avg_size - avg_size_mean) / avg_size_std < -threshold/2:
