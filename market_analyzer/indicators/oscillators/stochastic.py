...
def calculate_stochastic_fast(
    ...
    return {"fast_k": result["k"], "fast_d": result["d"]}
...
def calculate_stochastic_divergence(
    ...
    for i in range(1, len(recent_data) - 1):
        ...
        if (
            stoch is not None
            and prev_stoch is not None
            and next_stoch is not None
            and stoch > prev_stoch
            and stoch > next_stoch
        ):
    ...
    if len(price_lows) >= 2 and len(stoch_lows) >= 2:
        ...
        if (
            ...
            and stoch_val1 is not None
            and stoch_val2 is not None
            and price_val1 < price_val2
            and stoch_val1 > stoch_val2
        ):
...
def _calculate_stochastic_manual(
    ...
    return {"k": k_final, "d": d_final}


def _calculate_stochastic_series_manual(
    highs: np.ndarray,
    lows: np.ndarray,
    closes: np.ndarray,
    k_period: int,
...
def calculate_stochastic_signal(
    ...
    if len(closes) >= max(k_period, d_period) + 6:
        prev_stoch = calculate_stochastic(
            highs[:-1], lows[:-1], closes[:-1], k_period, 1, d_period
        )
