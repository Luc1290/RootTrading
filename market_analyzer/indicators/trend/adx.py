...
def calculate_dmi(
    ...
    return {"plus_di": result["plus_di"], "minus_di": result["minus_di"]}
    ...
    if TALIB_AVAILABLE:
        try:
            ...
            return {
                ...
                "plus_di": [
                    float(val) if not np.isnan(val) else None for val in plus_di
                ],
                "minus_di": [
                    float(val) if not np.isnan(val) else None for val in minus_di
                ],
            }
        except Exception as e:
