...
def calculate_confluence_score(indicators: Dict, 
                              current_price: Optional[float] = None,
                              confluence_type: ConfluenceType = ConfluenceType.MONO_TIMEFRAME) -> float:
    """
    ...
    try:
        ...
        logger.debug(f"📊 Confluence calculée: {confluence_score:.1f}% ({len(signals)} signaux)")
...
def _add_ma_signals(indicators: Dict, current_price: Optional[float], 
                   signals: list, weights: list) -> None:
...
def calculate_multi_timeframe_confluence(signal_strength: str,
                                       trend_alignment: str,
                                       confidence: float,
    ...
    risk_penalty = {
        ...
        "medium": 0.9,
        "high": 0.7
    }.get(risk_level.lower(), 0.9)

    raw_score = (0.4 * strength_score + 0.3 * alignment_score + 0.3 * confidence_score) * risk_penalty
