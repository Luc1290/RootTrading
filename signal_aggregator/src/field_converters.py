...
class FieldConverter:
    ...
    # Mapping des noms de champs de la DB vers les noms attendus par les
    # validators
    FIELD_NAME_MAPPING = {
    ...
    def convert_indicators(cls, indicators: dict[str, Any]) -> dict[str, Any]:
        ...
        if (
            "volume_quality_score" in mapped_indicators
            and "vol_quality" not in mapped_indicators
        ):
            mapped_indicators["vol_quality"] = mapped_indicators["volume_quality_score"]
            logger.debug(
                "FieldConverter: Added reverse mapping volume_quality_score -> vol_quality"
            )
        elif (
            "vol_quality" in mapped_indicators
            and "volume_quality_score" not in mapped_indicators
        ):
            mapped_indicators["volume_quality_score"] = mapped_indicators["vol_quality"]
            logger.debug(
                "FieldConverter: Added reverse mapping vol_quality -> volume_quality_score"
            )
        ...
        for key, value in mapped_indicators.items():
            try:
                ...
                if "strength" in key or "STRENGTH" in key:
                ...
                elif key in {"regime_conf", "pattern_conf"}:
                    # Ces champs sont déjà numériques mais peuvent nécessiter
                    # /100
                    converted[key] = (
                        cls._ensure_float(value) / 100.0
                        if cls._ensure_float(value) > 1
                        else cls._ensure_float(value)
                    )
                elif key == "break_prob":
                    # Déjà entre 0 et 1
                    ...
                    # Score sur 100 - GARDER le format 0-100 pour les
                    # validators
                    converted[key] = cls._ensure_float(value)
        ...
        # Étape 5: Ajout de champs Z-Score spécialisés pour validators
        # spécifiques
    ...
    def _add_calculated_fields(
        cls, converted: dict[str, Any], original: dict[str, Any]
    ):
        """Ajoute des champs calculés nécessaires aux validators."""

        # Calcul du score de force de régime numérique
        if "regime_strength" in converted and isinstance(
            converted["regime_strength"], float
        ):
            # Déjà converti, pas besoin de recalcul
            pass

        # Calcul du score de tendance numérique
        if "trend_strength" in converted and isinstance(
            converted["trend_strength"], float
        ):
            # Déjà converti
            pass
        ...
        try:
            ...
            if rsi_14 is not None:
                try:
                    ...
                    # RSI 50 = Z-Score 0, RSI 100 = Z-Score +3, RSI 0 = Z-Score
                    # -3
                    zscore_rsi = (rsi_val - 50) / 50 * 3
                    indicators["zscore_rsi"] = zscore_rsi
                    logger.debug(
                        f"FieldConverter: Created zscore_rsi={zscore_rsi:.2f} from rsi={rsi_val:.1f}"
                    )
                except (ValueError, TypeError):
                    pass
            ...
            if williams_r is not None:
                try:
                    ...
                    logger.debug(
                        f"FieldConverter: Created zscore_williams={zscore_williams:.2f} from williams_r={williams_val:.1f}"
                    )
                except (ValueError, TypeError):
                    pass
            ...
            if cci_20 is not None:
                try:
                    ...
                    # CCI normalement entre -200 et +200, normaliser vers
                    # Z-Score -3 à +3
                    # 200/3 = 66.67
                    zscore_cci = max(-3, min(3, cci_val / 66.67))
                    indicators["zscore_cci"] = zscore_cci
                    logger.debug(
                        f"FieldConverter: Created zscore_cci={zscore_cci:.2f} from cci_20={cci_val:.1f}"
                    )
                except (ValueError, TypeError):
                    pass
            ...
            if macd_histogram is not None:
                try:
                    ...
                    logger.debug(
                        f"FieldConverter: Created zscore_macd={zscore_macd:.2f} from macd_histogram={macd_val:.2f}"
                    )
                except (ValueError, TypeError):
                    pass
    ...
    def _add_specialized_zscore_fields(
        ...
        """
        try:
            # 1. price_zscore - utiliser zscore_20 comme proxy ou créer depuis
            # BB position
            if "price_zscore" not in indicators or indicators["price_zscore"] is None:
                if "zscore_20" in indicators and indicators["zscore_20"] is not None:
                    ...
                    "bb_position" in indicators
                    and indicators["bb_position"] is not None
                ):
                    try:
                        bb_pos = float(indicators["bb_position"])
                        indicators["price_zscore"] = (
                            bb_pos - 0.5
                        ) * 6  # Conversion BB vers Z-Score
                    except (ValueError, TypeError):
            ...
            if "volume_zscore" not in indicators or indicators["volume_zscore"] is None:
                if (
                    "relative_volume" in indicators
                    and indicators["relative_volume"] is not None
                ):
                    try:
                        rel_vol = float(indicators["relative_volume"])
                        # Normaliser relative_volume (1.0 = normal) vers Z-Score
                        # rel_vol > 1 = Z-Score positif, rel_vol < 1 = Z-Score
                        # négatif
                        indicators["volume_zscore"] = max(
                            -3, min(3, (rel_vol - 1.0) * 3)
                        )
                    except (ValueError, TypeError):
                        indicators["volume_zscore"] = 0.0
                elif (
                    "volume_ratio" in indicators
                    and indicators["volume_ratio"] is not None
                ):
                    try:
                        vol_ratio = float(indicators["volume_ratio"])
                        indicators["volume_zscore"] = max(
                            -3, min(3, (vol_ratio - 1.0) * 2.5)
                        )
                    except (ValueError, TypeError):
                        indicators["volume_zscore"] = 0.0
                else:
            ...
            if (
                "returns_zscore" not in indicators
                or indicators["returns_zscore"] is None
            ):
                if (
            ...
            if "distribution_normality" not in indicators:
                ...
                if regime == "RANGING":
                    indicators["distribution_normality"] = (
                        0.8  # Marché ranging = plus normal
                    )
                elif regime in ["TRENDING_BULL", "TRENDING_BEAR"]:
                    indicators["distribution_normality"] = (
                        0.6  # Trending = moins normal
                    )
                elif regime in ["VOLATILE", "BREAKOUT_BULL", "BREAKOUT_BEAR"]:
                    indicators["distribution_normality"] = (
                        0.4  # Volatile = très anormal
                    )
                else:
            ...
            if "statistical_confluence" not in indicators:
                ...
                if (
                    "confluence_score" in indicators
                    and indicators["confluence_score"] is not None
                ):
                    try:
    ...
    def _add_specialized_volume_spike_fields(
        ...
        """
        try:
            # 1. current_volume_spike - utiliser volume_spike_multiplier si
            # disponible
            if "current_volume_spike" not in indicators:
                if (
                    "volume_spike_multiplier" in indicators
                    and indicators["volume_spike_multiplier"] is not None
                ):
                    indicators["current_volume_spike"] = indicators[
            ...
            # 2. spike_quality_score - créer depuis volume_quality_score ou
            # relative_volume
            if "spike_quality_score" not in indicators:
                if (
                    "volume_quality_score" in indicators
                    and indicators["volume_quality_score"] is not None
                ):
                    try:
                    ...
                    try:
                        rel_vol = float(indicators["relative_volume"])
                        # Si relative_volume > 1.5, considérer comme bonne
                        # qualité
                        if rel_vol >= 2.0:
            ...
            if "spike_duration_bars" not in indicators:
                ...
                try:
                    ...
                    if spike_val >= 3.0:
                        indicators["spike_duration_bars"] = (
                            2  # Spike fort, durée courte
                        )
                    elif spike_val >= 2.0:
            ...
            if "time_since_spike" not in indicators:
                # Considérer spike comme actuel
                indicators["time_since_spike"] = 0

            # 5. relative_spike_strength - utiliser volume_spike_multiplier
            # normalisé
            if "relative_spike_strength" not in indicators:
                ...
                try:
                    ...
                    if spike_val >= 4.0:
                        # Spike maximal
                        indicators["relative_spike_strength"] = 1.0
                    elif spike_val >= 2.0:
                        indicators["relative_spike_strength"] = (
                            spike_val - 1.0
                        ) / 3.0  # Normalisation
                    else:
                        # Pas de spike
