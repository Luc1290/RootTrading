...
class CriticalFilters:
    ...
    def __init__(self, db_connection=None):
        ...
        self.fallback_max_atr_percent = (
            20.0  # Fallback 20% si calcul échoue (PEPE-friendly)
        )
        ...
        self.min_volume_ratio = (
            0.30  # Volume < 30% moyenne = marché mort (durci pour scalping)
        )
        self.min_volume_quality = (
            20  # Quality < 20% = données douteuses (durci pour scalping)
        )

        # Filtres conflits multi-timeframe - ajustés selon analyse
        self.max_conflicting_directions = 0.7  # > 70% contradictoire = problème
        self.min_mtf_consistency = (
            0.4  # 40% minimum (évite les cas 60% sell vs 40% buy)
        )
        ...
        self.pullback_tolerance = (
            self.pullback_tolerance_bp / 10000
        )  # Conversion en ratio
        ...
        if self.strict_mtf_enabled:
            ...
            apply_full_mtf = (
                ...
                or (
                    ...
                    and market_regime in ["BREAKOUT_BEAR", "VOLATILE"]
                )
                or (market_regime == "UNKNOWN" and consensus_strength < 0.6)
            )

            if apply_full_mtf:
                # MODE STRICT : Tous les filtres MTF (protection maximale)
                logger.info(
                    f"🛡️ MODE MTF STRICT activé: vol={volatility_level}, regime={market_regime}"
                )
                ...
                logger.info(
                    "✅ Filtres volatilité 15m et timing 3m SKIPPÉS (mode allégé)"
                )
    ...
    def _check_market_regime_compatibility(
        ...
        if signal_side == "BUY":
            ...
            if market_regime in ["TRENDING_BEAR", "BREAKOUT_BEAR"]:
                ...
                if (
                    volume_ratio and float(volume_ratio) > 1.5
            ...
            if market_regime == "UNKNOWN":
                ...
                if total_indicators > 0 and bearish_indicators / total_indicators > 0.5:
                    return (
                        False,
                        f"Achat rejeté: {bearish_indicators}/{total_indicators} indicateurs baissiers en régime {market_regime}",
                    )
            ...
            if (
                ...
                if momentum_score and float(momentum_score) > 70:
                    return (
                        False,
                        f"Vente risquée: {market_regime} fort (confidence {regime_confidence:.0f}%) avec momentum {momentum_score:.0f}",
                    )
        ...
        try:
            ...
            if bb_width is not None and bb_middle is not None and bb_middle > 0:
                ...
                if bb_width_percent > self.extreme_bb_width_threshold:
                    return (
                        False,
                        f"BB width {bb_width_percent:.2%} > {self.extreme_bb_width_threshold:.1%}",
                    )
        ...
        try:
            ...
            if volume_ratio is not None:
                ...
                if vol_ratio < self.min_volume_ratio:
                    return (
                        False,
                        f"Volume mort: ratio {vol_ratio:.2f} < {self.min_volume_ratio} (relatif: {relative_volume:.2f})",
                    )
        ...
        try:
            ...
            if total_count > 0:
                ...
                if max_direction_ratio < self.min_mtf_consistency:
                    return (
                        False,
                        f"Cohérence directionnelle {max_direction_ratio:.1%} < {self.min_mtf_consistency:.1%} ({buy_count}B/{sell_count}S sur {total_count})",
                    )
    ...
    def _get_dynamic_atr_threshold(self) -> float:
        ...
        try:
            if not self.db_connection:
                logger.warning(
                    "Pas de connexion DB disponible, utilisation fallback ATR"
                )
            ...
            with self.db_connection.cursor(
                ...
                if result and result["median_natr"]:
                    ...
                    logger.debug(
                        f"ATR dynamique: médiane {median_natr:.2f}%, P90 {p90_natr:.2f}%, seuil {dynamic_threshold:.1f}%"
                    )
    ...
    def _check_htf_direction(
        ...
        try:
            ...
            ema20_15m = context.get(
                "htf_ema20_15m"
            )  # Utilise ema_26 comme proxy pour ema_20
            ema100_15m = context.get(
                "htf_ema100_15m"
            )  # Utilise ema_99 comme proxy pour ema_100
            ...
            if signal_side != htf_direction:
                ...
                if self._check_htf_reversal_window(
                    signal_side, context, close_15m, ema20_15m, ema100_15m
                ):
                ...
                return (
                    False,
                    f"Signal {signal_side} contre tendance 15m {htf_direction} (fail-safe: {failsafe_reason})",
                )
    ...
    def _check_mtf_volatility(self, context: dict[str, Any]) -> tuple[bool, str]:
        ...
        try:
            ...
            if atr_ratio < self.mtf_min_atr_ratio:
                return (
                    False,
                    f"ATR 15m faible: {atr_ratio:.2f}x < {self.mtf_min_atr_ratio}x moyenne",
                )
    ...
    def _check_ltf_pullback_timing(
        ...
        try:
            ...
            if (
                ...
                and consensus_regime in ["TRENDING_BULL", "BREAKOUT_BULL"]
                and signal_side == "BUY"
            ):

                logger.info(
                    f"🚀 MOMENTUM FAST-LANE activée: consensus {consensus_strength:.2f}, régime {consensus_regime}"
                )
                return (
                    True,
                    f"Momentum fast-lane: {consensus_strength:.2f} @ {consensus_regime}",
                )
            ...
            current_price = (
                float(current_price_val) if current_price_val is not None else 0.0
            )
            ...
            logger.info(
                f"📊 Tolérance pullback calculée: {tolerance_bp}bp (base:{base_tolerance*10000:.0f}bp → adaptive:{adaptive_tolerance*10000:.0f}bp → final:{tolerance_bp}bp, ATR floor:{atr_floor_pct*10000:.0f}bp)"
            )
            ...
            if bars_since_touch is not None and bars_since_touch <= 8:
                logger.info(
                    f"📈 RECENT TOUCH: Prix a touché EMA20 il y a {bars_since_touch} bougies"
                )
    ...
    def _check_min_risk_reward(self, context: dict[str, Any]) -> tuple[bool, str]:
        ...
        try:
            ...
            if current_price and nearest_support:
                ...
                swing_distance = (
                    abs(float(cp_val) - float(ns_val))
                    if cp_val is not None and ns_val is not None
                    else 0.0
                )
                ...
                swing_distance = (
                    abs(float(cp_val) - float(nr_val))
                    if cp_val is not None and nr_val is not None
                    else 0.0
                )
            ...
            if risk_reward < min_rr:
                return (
                    False,
                    f"R/R {risk_reward:.2f} < {min_rr:.1f} (regime:{market_regime}, vol:{volatility_level})",
                )
    ...
    def _check_failsafe_ltf_alignment(
        ...
        try:
            ...
            if consensus_strength < min_consensus:
                return (
                    False,
                    f"Fail-safe: consensus {consensus_strength:.2f} < {min_consensus:.2f} requis (vol:{volatility_level})",
                )
            ...
            consensus_regime = context.get(
                "consensus_regime", market_regime
            )  # Utiliser consensus_regime si dispo

            # Seuils adaptatifs selon consensus strength
            momentum_threshold_buy = (
                55 if consensus_strength >= 0.80 else 52
            )  # Assoupli
            momentum_threshold_sell = 45 if consensus_strength >= 0.80 else 48

            logger.info(
                f"📊 Seuils momentum calculés: BUY>{momentum_threshold_buy}, SELL<{momentum_threshold_sell} (consensus: {consensus_strength:.2f})"
            )
            ...
            if signal_side == "BUY":
                ...
                strong_rebound = (
                    consensus_strength >= 0.80
                    and momentum_ok
                    and float(momentum_score) > 50
                )  # Au-dessus de la neutralité
                ...
                strong_breakdown = (
                    ...
                    and momentum_ok
                    and float(momentum_score) < 50
                )  # En-dessous de la neutralité

                ltf_aligned = bias_ok or strong_breakdown

                if (
                    consensus_regime in ["TRENDING_BEAR", "BREAKOUT_BEAR"]
                    and consensus_strength >= 0.80
                ):
                    ltf_aligned = True
            ...
            if current_atr and avg_atr:
                ...
                if atr_ratio < required_atr:
                    return (
                        False,
                        f"ATR ratio {atr_ratio:.2f} < {required_atr:.2f}x requis (regime:{market_regime}, consensus:{consensus_strength:.2f})",
                    )
    ...
    def _check_htf_reversal_window(
        ...
        try:
            ...
            if not (
                ...
                and wave_winner
                and total_strategies >= 10
                and signal_side == "BUY"
                and consensus_regime in ["TRENDING_BULL", "TRANSITION", "RANGING"]
            ):
                return False
    ...
    def _check_momentum_bypass(
        ...
        try:
            ...
            if volume_ratio < 1.3:
                return (
                    False,
                    f"Momentum bypass: volume {volume_ratio:.2f} < 1.3x requis",
                )

            if current_price <= ema100_3m:
                return (
                    False,
                    f"Momentum bypass: prix {current_price:.4f} ≤ EMA100 {ema100_3m:.4f}",
                )
            ...
            if ema20_3m:
                ...
                if ema20_vs_ema100_ratio < 1.001:  # EMA20 pas vraiment au-dessus EMA100
                    return (
                        False,
                        f"Momentum bypass: EMA20 pas en pente positive ({ema20_vs_ema100_ratio:.4f})",
                    )
            ...
            if distance_pct > max_overshoot:
                return (
                    False,
                    f"Momentum bypass: dépassement {distance_pct:.4f} > {max_overshoot:.4f} max",
                )

            logger.info(
                f"🚀 MOMENTUM BYPASS activé: distance={distance_pct:.4f}, "
