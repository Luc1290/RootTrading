...
class DatabaseManager:
    ...
    def _prepare_signal_data(self, validated_signal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            metadata = {
                ...
                'weighted_validation_score': validated_signal.get('weighted_validation_score'),
    ...
    def _extract_price(self, validated_signal: Dict[str, Any]) -> float:
        ...
        try:
            ...
            # Valeur par défaut
            logger.warning(f"Prix non trouvé pour signal {validated_signal.get('symbol')}, utilisation de 0.0")
            return 0.0
    ...
    def _parse_timestamp(self, timestamp_str: str) -> datetime:
        ...
        try:
            ...
            logger.warning(f"Format timestamp non reconnu: {timestamp_str}, utilisation timestamp actuel")
            return datetime.utcnow()
    ...
    def store_multiple_signals(self, validated_signals: List[Dict[str, Any]]) -> List[Optional[int]]:
        """
        ...
        try:
            with self.db_connection.cursor() as cursor:
                ...
                logger.info(f"Batch de {successful_stores}/{len(validated_signals)} signaux stocké en DB")
                ...
                for row in rows:
                    ...
                    if signal_dict.get('metadata'):
                        try:
                            ...
                            logger.warning(f"Erreur parsing métadonnées pour signal {signal_dict['id']}")
    ...
    def get_storage_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'success_rate': (
                self.stats['signals_stored'] / (self.stats['signals_stored'] + self.stats['storage_errors'])
                if (self.stats['signals_stored'] + self.stats['storage_errors']) > 0 else 1.0
            )
        }
