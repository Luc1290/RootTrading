...
class Volume_Buildup_Validator(BaseValidator):
    ...
    def validate_signal(self, signal: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if volume_ratio is not None and volume_ratio < self.min_volume_ratio:
                logger.debug(f"{self.name}: Volume insuffisant ({volume_ratio:.2f}x) pour {self.symbol}")
                if signal_confidence < 0.8:
                    return False
                    
            # 2. Validation trend volume
            if volume_trend is not None and volume_trend < self.min_volume_trend:
                logger.debug(f"{self.name}: Trend volume décroissant ({volume_trend:.2f}) pour {self.symbol}")
            ...
            if accumulation_distribution_score is not None:
                if signal_side == "BUY" and accumulation_distribution_score < self.min_accumulation_score:
                    logger.debug(f"{self.name}: Score accumulation insuffisant ({accumulation_distribution_score:.2f}) pour BUY {self.symbol}")
                    if signal_confidence < 0.7:
                        return False
                elif signal_side == "SELL" and accumulation_distribution_score > -self.min_accumulation_score:
                    logger.debug(f"{self.name}: Score distribution insuffisant ({accumulation_distribution_score:.2f}) pour SELL {self.symbol}")
                    if signal_confidence < 0.7:
                        return False
            ...
            if buy_sell_pressure is not None:
                if signal_side == "BUY" and buy_sell_pressure < self.min_buy_pressure:
                    logger.debug(f"{self.name}: Pression acheteuse insuffisante ({buy_sell_pressure:.2f}) pour {self.symbol}")
                    if signal_confidence < 0.6:
                        return False
                elif signal_side == "SELL" and buy_sell_pressure > (1 - self.min_buy_pressure):
                    logger.debug(f"{self.name}: Pression vendeuse insuffisante ({1-buy_sell_pressure:.2f}) pour {self.symbol}")
                    if signal_confidence < 0.6:
                        return False
            ...
            if volume_buildup_slope is not None and volume_buildup_slope < self.buildup_slope_threshold:
                logger.debug(f"{self.name}: Pente buildup insuffisante ({volume_buildup_slope:.3f}) pour {self.symbol}")
                if signal_confidence < 0.6:
                    return False
                    
            if volume_buildup_consistency is not None and volume_buildup_consistency < self.buildup_consistency:
                logger.debug(f"{self.name}: Consistance buildup insuffisante ({volume_buildup_consistency:.2f}) pour {self.symbol}")
                if signal_confidence < 0.6:
                    return False
                    
            # 6. Validation liquidité
            if liquidity_score is not None and liquidity_score < self.min_liquidity_score:
                logger.debug(f"{self.name}: Score liquidité insuffisant ({liquidity_score:.2f}) pour {self.symbol}")
                if signal_confidence < 0.5:
                    return False
                    
            if spread_ratio is not None and spread_ratio > self.max_spread_ratio:
                logger.debug(f"{self.name}: Spread excessif ({spread_ratio*100:.3f}%) pour {self.symbol}")
                if signal_confidence < 0.6:
                    return False
                    
            if bid_ask_ratio is not None and bid_ask_ratio < self.min_bid_ask_ratio:
                logger.debug(f"{self.name}: Ratio bid/ask défavorable ({bid_ask_ratio:.2f}) pour {self.symbol}")
                if signal_confidence < 0.5:
                    return False
            ...
            if obv_trend is not None:
                if signal_side == "BUY" and obv_trend < self.min_obv_trend:
                    logger.debug(f"{self.name}: OBV trend baissier ({obv_trend:.2f}) pour BUY {self.symbol}")
                    if signal_confidence < 0.7:
                        return False
                elif signal_side == "SELL" and obv_trend > -self.min_obv_trend:
                    logger.debug(f"{self.name}: OBV trend haussier ({obv_trend:.2f}) pour SELL {self.symbol}")
                    if signal_confidence < 0.7:
                        return False
            ...
            if obv_price_divergence is not None and abs(obv_price_divergence) > self.obv_divergence_threshold:
                logger.debug(f"{self.name}: Divergence OBV/prix excessive ({obv_price_divergence:.2f}) pour {self.symbol}")
                if signal_confidence < 0.6:
                    return False
            ...
            if money_flow_index is not None:
                if signal_side == "BUY" and money_flow_index < self.min_money_flow:
                    logger.debug(f"{self.name}: Money flow insuffisant ({money_flow_index:.2f}) pour BUY {self.symbol}")
                    if signal_confidence < 0.6:
                        return False
                elif signal_side == "SELL" and money_flow_index > (1 - self.min_money_flow):
                    logger.debug(f"{self.name}: Money flow excessif ({money_flow_index:.2f}) pour SELL {self.symbol}")
                    if signal_confidence < 0.6:
                        return False
            ...
            if volume_volatility is not None and volume_volatility > self.max_volume_volatility:
                logger.debug(f"{self.name}: Volatilité volume excessive ({volume_volatility:.2f}) pour {self.symbol}")
                if signal_confidence < 0.6:
                    return False
    ...
    def get_validation_score(self, signal: Dict[str, Any]) -> float:
        ...
        try:
            ...
            volume_trend = float(self.context.get('volume_trend', 0)) if self.context.get('volume_trend') is not None else 0
        ...
        try:
            ...
            if is_valid:
                ...
                if volume_ratio and volume_ratio < self.min_volume_ratio:
                    ...
                    return f"{self.name}: Rejeté - Score distribution insuffisant ({accumulation_distribution_score:.2f})"
                elif buy_sell_pressure:
                    if signal_side == "BUY" and buy_sell_pressure < self.min_buy_pressure:
                        return f"{self.name}: Rejeté - Pression acheteuse insuffisante ({buy_sell_pressure:.2f})"
                    elif signal_side == "SELL" and buy_sell_pressure > (1 - self.min_buy_pressure):
                        return f"{self.name}: Rejeté - Pression vendeuse insuffisante ({1-buy_sell_pressure:.2f})"
                    ...
                    return f"{self.name}: Rejeté - Liquidité insuffisante ({liquidity_score:.2f})"
