...
class BinanceAccountManager:
        ...
        while retry_count < max_retries:
            try:
                ...
                try:
                    ...
                    error_message = (
                        f"{error_message}: {error_data.get('msg', str(error_data))}"
                    )
                except Exception:
    ...
    def get_ticker_for_symbol(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if isinstance(result, list):
                raise BinanceApiError(
                    "Réponse inattendue: liste au lieu de dict pour ticker symbol"
                )
            return result
    ...
    def calculate_asset_values(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            while retry < max_retries and not balances:
                try:
                    ...
                    if not balances and retry < max_retries - 1:
                        ...
                        wait_time = 2**retry
                        logger.warning(
                            f"⚠️ Aucune balance reçue, nouvelle tentative {retry}/{max_retries} dans {wait_time}s..."
                        )
                        time.sleep(wait_time)
                except Exception as e:
                    retry += 1
            ...
            if not balances:
                logger.error(
                    "❌ Impossible de récupérer les balances après plusieurs tentatives"
                )
                return []
            ...
            while retry < max_retries and not prices:
                try:
                    ...
                    if not prices and retry < max_retries - 1:
                        ...
                        wait_time = 2**retry
                        logger.warning(
                            f"⚠️ Aucun prix reçu, nouvelle tentative {retry}/{max_retries} dans {wait_time}s..."
                        )
                        time.sleep(wait_time)
                except Exception as e:
                    retry += 1
            ...
            if not prices:
                logger.error(
                    "❌ Impossible de récupérer les prix après plusieurs tentatives"
                )
...
if __name__ == "__main__":
    ...
    try:
        ...
        for balance in balances[
            :5
        ]:  # Afficher les 5 premières pour éviter une sortie trop grande
        ...
        for balance in valued_balances:
            if "value_usdc" in balance and balance["value_usdc"] is not None:
                ...
                if (
                    balance["value_usdc"] > 10
                ):  # N'afficher que les valeurs significatives
                    print(
                        f"{balance['asset']}: {balance['total']} ({balance['value_usdc']:.2f} USDC)"
                    )
