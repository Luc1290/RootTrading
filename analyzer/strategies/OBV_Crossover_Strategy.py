...
class OBV_Crossover_Strategy(BaseStrategy):
        ...
        if rsi_14 is not None:
            try:
                ...
                if (signal_side == "BUY" and 30 <= rsi <= 65) or (
                    signal_side == "SELL" and 35 <= rsi <= 70
                ):  # Assoupli (60 -> 65)
                    confirmations += 1
    ...
    def generate_signal(self) -> dict[str, Any]:
        ...
        is_valid, error_response, values, obv, obv_ma, obv_distance = (
            self._validate_obv_data()
        )
        if not is_valid:
        ...
        confirmations_count, confirmation_details = self._count_confirmations(
            values, signal_side
        )
        ...
        if confirmations_count < self.min_confirmations_required:
            confirmations_penalty = -0.10 * (
                self.min_confirmations_required - confirmations_count
            )
            reason += (
                f" ({confirmations_count}/{self.min_confirmations_required} confirm.)"
            )
        ...
        confirmation_bonus = min(
            confirmations_count * 0.05, 0.20
        )  # Max 20% pour 4+ confirmations (légèrement augmenté)
        ...
        if market_regime:
            ...
            if (
                signal_side == "BUY"
                and regime_str in ["TRENDING_BULL", "BREAKOUT_BULL"]
            ) or (
                signal_side == "SELL"
                and regime_str in ["TRENDING_BEAR", "BREAKOUT_BEAR"]
            ):
                confidence_boost += 0.10
                ...
                confidence_boost += (
                    0.08  # Bonus en range - OBV parfait pour accumulation
                )
