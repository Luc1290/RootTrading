...
class Liquidity_Sweep_Buy_Strategy(BaseStrategy):
    ...
    def _get_price_volume_data(self) -> dict[str, float | None]:
    ...
    def _detect_liquidity_sweep_setup(
        ...
        (support_level - current_low) / support_level
        recent_sweep = False
        sweep_bars_ago = 0
        ...
        lows = [
            ...
            prev_low_1,
            prev_low_2,
            price_data.get("prev_low_3"),
            price_data.get("prev_low_4"),
            price_data.get("prev_low_5"),
            price_data.get("prev_low_6"),
        ...
        if recent_sweep and sweep_bars_ago > self.max_sweep_duration:
            return {
                "is_sweep": False,
                "reason": f"Sweep trop ancien ({sweep_bars_ago} barres > {self.max_sweep_duration})",
            }
        ...
        recovery_successful = (
            ...
            and current_price > support_level * (1 + self.recovery_threshold)
        )

        if not recovery_successful:
            return {
                "is_sweep": False,
                "reason": "Pas encore de recovery au-dessus support",
            }
        ...
        if not sweep_lows:
            ...
            max_sweep_distance = max(
                abs(low - support_level) / support_level for low in sweep_lows
            )
    ...
    def validate_data(self) -> bool:
        ...
        for key in required_ohlcv:
            if (
                ...
                or len(self.data[key]) < 8
            ):
                logger.warning(
                    f"{self.name}: Données {key} insuffisantes pour liquidity sweep (besoin 8 barres)"
                )
                return False
