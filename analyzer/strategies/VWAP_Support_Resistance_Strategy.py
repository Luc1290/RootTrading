...
class VWAP_Support_Resistance_Strategy(BaseStrategy):
            ...
            try:
                ...
                if vol_ratio >= self.strong_vwap_volume_threshold:  # >= 2.0x
                    ...
                    return {
                        'is_bounce': False,
                        'score': 0.0,
                        'indicators': [f'Volume insuffisant ({vol_ratio:.1f}x) < 1.4x requis'],
                        'vwap_level': vwap_val
                    }
            except (ValueError, TypeError):
                pass
    ...
    def _detect_momentum_alignment(self, values: Dict[str, Any], signal_direction: str) -> Dict[str, Any]:
        ...
        if momentum_val is not None:
            try:
                ...
                if signal_direction == "BUY" and momentum >= (50 + self.min_momentum_threshold * 50):  # >=55 (10%)
                    momentum_score += 30
                    momentum_indicators.append(f"Momentum haussier fort ({momentum:.1f})")
                elif signal_direction == "SELL" and momentum <= (50 - self.min_momentum_threshold * 50):  # <=45 (10%)
                    momentum_score += 30
        ...
        if directional_bias:
            if (signal_direction == "BUY" and directional_bias.upper() == 'BULLISH') or \
        ...
        if rsi_14 is not None:
            try:
                ...
                elif signal_direction == "SELL" and 40 <= rsi_val <= 65:  # Zone resserrée
                    momentum_score += 15
    ...
    def generate_signal(self) -> Dict[str, Any]:
            ...
            try:
                ...
                if signal_side == "BUY":
                    ...
                    if volatility_regime == "low" and atr_percentile < 30:
                        ...
                        confidence_boost += 0.08
                        reason += f" + volatilité normale support ({atr_percentile:.0f}%)"
                    ...
                    if volatility_regime == "low" and atr_percentile < 25:
                        ...
                        reason += f" + volatilité optimale résistance ({atr_percentile:.0f}%)"
                    elif volatility_regime == "high" and atr_percentile > 70:
                        if atr_percentile > 85:  # Volatilité très élevée = continuation baissière
            ...
            if signal_side == "BUY":
                ...
                if market_regime == "RANGING":
                    if regime_str == "EXTREME":  # Range très défini
                        confidence_boost += 0.25  # BONUS AUGMENTÉ - VWAP résistance excellent
                        if len(reason_elements) < 4:
        ...
        confidence = min(1.0, max(0.0, self.calculate_confidence(base_confidence, 1.0 + confidence_boost)))
