...
class Spike_Reaction_Buy_Strategy(BaseStrategy):
    ...
    def __init__(self, symbol: str, data: dict[str, Any], indicators: dict[str, Any]):
        ...
        # RSI survente extrême crypto (était 18)
        self.extreme_oversold_threshold = 20
        # Williams %R survente assoupli (était -82)
        self.williams_r_oversold = -78
        ...
        # Momentum reversal ACCESSIBLE (+8%)
        self.momentum_reversal_threshold = 48
        # Momentum reversal fort RÉALISTE (+3%)
        self.strong_momentum_reversal = 58
    ...
    def _detect_price_spike_down(self, values: dict[str, Any]) -> dict[str, Any]:
            ...
            try:
                ...
                if roc_val <= self.extreme_price_drop:
                    ...
                    spike_indicators.append(
                        f"ROC10 chute significative ({roc_val*100:.1f}%)"
                    )
            except (ValueError, TypeError):
                pass
        ...
        if momentum_10 is not None:
            try:
                ...
                if momentum_val < -0.02:  # Momentum très négatif
                    spike_score += 0.2
                    spike_indicators.append(
                        f"Momentum très négatif ({momentum_val:.3f})"
                    )
                elif momentum_val < 0:
                    spike_score += 0.12
        ...
        return {
            "is_price_spike": spike_score
            >= 0.2,  # Seuil RÉVOLUTIONNÉ (+50% accessibilité)
            "score": spike_score,
    ...
    def _detect_volume_spike(self, values: dict[str, Any]) -> dict[str, Any]:
        ...
        if volume_ratio is not None:
            try:
                ...
                # Utiliser volume_ratio comme référence principale (plus
                # cohérent)
            ...
            try:
                ...
                if intensity >= 3.0:  # Seuil plus strict pour crypto
                    ...
                    volume_indicators.append(
                        f"Intensité TRÈS élevée ({intensity:.1f}x)"
                    )
                elif intensity >= 2.0:
                    volume_score += 0.1
        ...
        return {
            "is_volume_spike": volume_score
            >= 0.18,  # Seuil OPTIMISÉ (+39% accessibilité)
            "score": volume_score,
    ...
    def _detect_oversold_extreme(self, values: dict[str, Any]) -> dict[str, Any]:
            ...
            try:
                ...
                if k_val <= 15 and d_val <= 15:  # Plus strict
                    ...
                    oversold_indicators.append(
                        f"Stoch survente EXTRÊME (K={k_val:.1f}, D={d_val:.1f})"
                    )
                elif k_val <= 20 and d_val <= 20:
                    oversold_score += 0.15
                    oversold_indicators.append(
                        f"Stoch survente (K={k_val:.1f}, D={d_val:.1f})"
                    )
            except (ValueError, TypeError):
                pass
        ...
        return {
            "is_oversold": oversold_score
            >= 0.28,  # Seuil RÉVOLUTIONNÉ (+25% accessibilité)
    ...
    def _detect_stabilization(self, values: dict[str, Any]) -> dict[str, Any]:
        ...
        # LOGIQUE COHÉRENTE : après crash, momentum remonte progressivement
        # (35-55 zone valide)
        momentum_score = values.get("momentum_score")
        if momentum_score is not None:
            try:
        ...
        if roc_10 is not None:
            try:
                ...
                if roc_val >= self.min_roc_improvement:
                    stabilization_score += 0.25
                    stabilization_indicators.append(
                        f"ROC amélioration ({roc_val*100:.1f}%)"
                    )
                elif roc_val >= -0.015:  # Amélioration partielle
                    stabilization_score += 0.15
                    stabilization_indicators.append(
                        f"ROC stabilisation ({roc_val*100:.1f}%)"
                    )
            except (ValueError, TypeError):
                pass
        ...
        if pattern_detected and "reversal" in str(pattern_detected).lower():
            ...
            stabilization_indicators.append("Pattern reversal détecté")
            ...
            if pattern_confidence is not None:
                try:
                    ...
                    if conf_val >= 70:
                        stabilization_score += 0.15
                        stabilization_indicators.append(
                            f"Pattern confidence {conf_val:.0f}%"
                        )
                except (ValueError, TypeError):
                    pass
        ...
        return {
            "is_stabilized": stabilization_score
            >= 0.25,  # Seuil OPTIMISÉ (+40% accessibilité)
    ...
    def generate_signal(self) -> dict[str, Any]:
        ...
        # Vérifier volume quality pour éviter bottom fishing de mauvaise
        # qualité
        ...
        # NOUVELLE LOGIQUE: Score pondéré flexible au lieu de conditions ET
        # strictes
        spike_total_score = (
            price_spike_analysis["score"] * 0.40  # Prix spike priorité maximum
            # Volume confirmation importante
            + volume_spike_analysis["score"] * 0.30
        ...
        # Signal BUY si score suffisant ET minimum 2 conditions (ASSOUPLI pour
        # crypto intraday)
        if spike_total_score >= 0.25 and conditions_met >= 2 and volume_quality_ok:
            ...
            return {
                ...
                "metadata": {
                    ...
                    "roc_10": values.get("roc_10"),
                    "rsi_14": values.get("rsi_14"),
                    "volume_spike_multiplier": values.get("volume_ratio")
                    or values.get(
                        "volume_spike_multiplier", 1.0
                    ),  # Utiliser volume_ratio comme référence
                    "momentum_score": values.get("momentum_score"),
        ...
        if not price_spike_analysis["is_price_spike"]:
            missing_conditions.append(
                f"Pas de spike prix (score: {price_spike_analysis['score']:.2f}/0.2)"
            )
        if not volume_spike_analysis["is_volume_spike"]:
            missing_conditions.append(
                f"Pas de spike volume (score: {volume_spike_analysis['score']:.2f}/0.18)"
            )
        if not oversold_analysis["is_oversold"]:
            missing_conditions.append(
                f"Pas survente extrême (score: {oversold_analysis['score']:.2f}/0.28)"
            )
        if not stabilization_analysis["is_stabilized"]:
            missing_conditions.append(
                f"Pas stabilisé (score: {stabilization_analysis['score']:.2f}/0.25)"
            )
        if not volume_quality_ok:
            missing_conditions.append(
                f"Volume quality insuffisant ({volume_quality_score:.1f} < {self.min_volume_quality})"
            )
        if spike_total_score < 0.25:
            missing_conditions.append(
                f"Score total insuffisant ({spike_total_score:.2f} < 0.25)"
            )
        if conditions_met < 2:
            missing_conditions.append(
                f"Conditions insuffisantes ({conditions_met}/4 < 2)"
            )
    ...
    def validate_data(self) -> bool:
        ...
        required_indicators = ["roc_10", "rsi_14", "volume_spike_multiplier"]
