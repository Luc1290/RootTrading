"""
ZScoreExtremeReversalStrategy - Stratégie de reversal basée sur les extrêmes Z-Score.
...
class ZScoreExtremeReversalStrategy(BaseStrategy):
    """
    ...
    def __init__(self, symbol: str, data: dict[str, Any], indicators: dict[str, Any]):
        ...
        # 2.8 (rare mais atteignable)
        self.ultra_extreme_zscore_threshold = 2.8

        # Paramètres Bollinger Bands PLUS FLEXIBLES
        # 85% au lieu de 92% (plus accessible)
        self.bb_extreme_position_threshold = 85
        ...
        # RSI < 20 ou > 80 (20/80 vs 15/85)
        self.rsi_very_extreme_threshold = 20
    ...
    def _calculate_zscore_proxy(
        ...
        if bb_position is not None:
            try:
                ...
                # Simple transformation linéaire : BB position vers Z-Score -2
                # à +2
                ...
                if zscore_from_bb > self.extreme_zscore_threshold:
                    # Surachat
                    zscore_data["zscore_direction"] = "extreme_positive"
                elif zscore_from_bb < -self.extreme_zscore_threshold:
                    # Survente
                    zscore_data["zscore_direction"] = "extreme_negative"
                else:
    ...
    def generate_signal(self) -> dict[str, Any]:
        ...
        if not reversal_analysis["is_reversal"]:
            return self._create_rejection_signal(
                f"Z-Score extrême mais reversal non confirmé (score: {reversal_analysis['score']:.2f})",
                {
                    "zscore_direction": zscore_data["zscore_direction"],
        ...
        if reversal_direction == "bullish":
            ...
            return self._create_rejection_signal("Direction reversal indéterminée")
        ...
        if volume_ratio is not None:
            try:
                ...
                if vol_ratio < self.min_volume_confirmation:  # <1.0
                    return self._create_rejection_signal(
                        f"Rejet ZScore: volume insuffisant ({vol_ratio:.1f}x)",
                        {"volume_ratio": vol_ratio},
                    )
                if vol_ratio >= self.exceptional_volume_threshold:  # >=2.0
                    confidence_boost += 0.15
                    reason += f" + volume exceptionnel ({vol_ratio:.1f}x)"
                elif vol_ratio >= self.strong_volume_threshold:  # >=1.5
            ...
            if nearest_support is not None:
                try:
                    ...
                    if distance_to_support <= 0.02:  # Près support
                        ...
                        if added_elements < 2:
                            reason_elements.append("près support")
                            added_elements += 1
                except (ValueError, TypeError):
                    pass
        else:  # SELL
            ...
            if nearest_resistance is not None:
                try:
                    ...
                    if distance_to_resistance <= 0.02:  # Près résistance
                        confidence_boost += 0.1
                        if added_elements < 2:
                            reason_elements.append("près résistance")
                            added_elements += 1
                except (ValueError, TypeError):
                    pass
        ...
        if adx_14 is not None:
            try:
                ...
                if adx_val > 45:  # Tendance trop forte pour reversal
                    return self._create_rejection_signal(
                        f"Rejet ZScore: tendance trop forte (ADX={adx_val:.1f})",
                        {"adx_14": adx_val},
                    )
        ...
        if market_regime == "RANGING":
            ...
            if regime_str in ["EXTREME", "STRONG"]:  # Trend trop fort
                return self._create_rejection_signal(
                    f"Rejet ZScore: trend extrême ({market_regime} {regime_str})",
                    {"market_regime": market_regime, "regime_strength": regime_str},
                )
        ...
        if (
            ...
            try:
                ...
                if pattern_conf > 70:
                    confidence_boost += 0.1
                    if added_elements < 2:
                        reason_elements.append("pattern")
                        added_elements += 1
            except (ValueError, TypeError):
                pass
