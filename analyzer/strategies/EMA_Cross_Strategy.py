    ...
    def generate_signal(self) -> Dict[str, Any]:
        ...
        if ema_50 is not None:
            if signal_side == "BUY" and current_price > ema_50 and ema_12 > ema_50:
                ...
                signal_side == "BUY" and (current_price < ema_50 or ema_12 < ema_50)
            ) or (
        ...
        if ema_distance_pct >= self.strong_separation_pct:  # 1.0%
            confidence_boost += 0.20
            reason += (
                f" - séparation FORTE ({self._format_percentage(ema_distance_pct)})"
            )
            cross_quality = "strong"
        elif ema_distance_pct >= 0.5:
            confidence_boost += 0.12
            reason += (
                f" - séparation correcte ({self._format_percentage(ema_distance_pct)})"
            )
            ...
            reason += (
                f" - séparation faible ({self._format_percentage(ema_distance_pct)})"
            )
            cross_quality = "weak"
