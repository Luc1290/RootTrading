...
class ROC_Threshold_Strategy(BaseStrategy):
    ...
    def generate_signal(self) -> dict[str, Any]:
        ...
        return self._create_roc_signal(
    ...
    def _create_roc_signal(
        ...
        if macd_line is not None and macd_signal is not None:
            try:
                ...
                if (signal_side == "BUY" and macd_bullish) or (
                    signal_side == "SELL" and not macd_bullish
                ):
            ...
            try:
                ...
                if signal_side == "BUY":
                    if wr > -20:  # Williams R overbought = momentum haussier fort
                        ...
                        wr < -80
                    ):  # Williams R oversold profond = potentiel mais risqué
        ...
        if volatility_regime == "high":
            if threshold_result["is_extreme"]:
                confidence_boost += (
                    0.05  # Momentum extrême en haute volatilité = signal fort
                )
                reason += " + volatilité élevée favorable"
            else:
                confidence_boost -= (
                    0.05  # Momentum normal en haute volatilité = moins fiable
                )
        ...
        return {
            ...
            "metadata": {
                ...
                "roc_value_capped": (
                    roc_value if was_capped else None
                ),  # Valeur cappée si applicable
