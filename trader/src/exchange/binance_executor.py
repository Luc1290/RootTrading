...
class BinanceExecutor:
    ...
    def __init__(self, api_key: str, api_secret: str, demo_mode: bool = False):
        ...
        logger.info(
            f"✅ BinanceExecutor initialisé en mode {'DÉMO' if demo_mode else 'RÉEL'}"
        )
    ...
    def _simulate_order(self, order: TradeOrder) -> TradeExecution:
        ...
        price = (
            ...
            else self.utils.get_current_price(order.symbol)
        )
        ...
        logger.info(
            f"✅ [DÉMO] Ordre simulé: {order.side.value if hasattr(order.side, 'value') else order.side} {order.quantity} {order.symbol} @ {price}"
        )
        return execution
