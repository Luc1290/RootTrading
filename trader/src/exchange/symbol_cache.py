...
class SymbolConstraintsCache:
    ...
    def get(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        with self._lock:
            ...
            if current_time - cached_time > self.ttl_seconds:
                logger.debug(f"⏰ Cache expiré pour {symbol} (âge: {current_time - cached_time:.1f}s)")
                # Nettoyer l'entrée expirée
                del self._cache[symbol]
                del self._timestamps[symbol]
                return None
            
            logger.debug(f"✅ Cache hit pour {symbol} (âge: {current_time - cached_time:.1f}s)")
    ...
    def cleanup_expired(self) -> int:
        ...
        with self._lock:
            ...
            if expired_symbols:
                logger.debug(f"🧹 Nettoyage cache: {len(expired_symbols)} entrées expirées supprimées")
