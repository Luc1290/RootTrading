    ...
    def validate_and_adjust_order(self, order: TradeOrder) -> TradeOrder:
        ...
        try:
            ...
            if adjusted_order.quantity <= 0:
                logger.error(f"❌ Quantité invalide (zéro ou négative): {adjusted_order.quantity}")
                raise OrderValidationError(f"Quantité invalide (zéro ou négative): {adjusted_order.quantity}")
            
            # Tronquer la quantité au step size
            original_quantity = adjusted_order.quantity
            adjusted_order.quantity = self.constraints.truncate_quantity(adjusted_order.symbol, adjusted_order.quantity)
            
            # Log pour débogage si la quantité a changé significativement
            if abs(original_quantity - adjusted_order.quantity) / original_quantity > 0.01:  # Changement de plus de 1%
                logger.warning(f"⚠️ Ajustement significatif de quantité: {original_quantity} → {adjusted_order.quantity} pour {adjusted_order.symbol}")
            ...
            if adjusted_order.quantity < min_qty:
                logger.error(f"❌ Quantité {adjusted_order.quantity} trop faible pour {adjusted_order.symbol} (min: {min_qty})")
                raise OrderValidationError(f"Quantité {adjusted_order.quantity} trop faible pour {adjusted_order.symbol} (min: {min_qty})")
            
            # Validation de prix (si c'est un ordre limité)
            if adjusted_order.price is not None:
                adjusted_order.price = self.constraints.round_price(adjusted_order.symbol, adjusted_order.price)
            
            # Validation de notional (quantité * prix)
            from trader.src.exchange.binance_utils import BinanceUtils
            binance_utils = BinanceUtils("", "")  # Clés vides car on n'utilise pas d'API ici
            ...
            if price is None:
                try:
                    ...
                    logger.warning(f"⚠️ Impossible de récupérer le prix actuel: {str(e)}")
            ...
            logger.info(f"✅ Ordre validé et ajusté: {adjusted_order.symbol} {adjusted_order.side} {adjusted_order.quantity}")
            ...
            logger.error(f"❌ Erreur inattendue lors de la validation de l'ordre: {str(e)}")
