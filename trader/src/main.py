...
class TraderService:
    ...
    def diagnostic(self):
        ...
        last_prices = self.order_manager.last_prices.copy() if hasattr(self.order_manager, 'last_prices') else {}
        
        # Vérifier l'état des connexions
        redis_status = "connected" if hasattr(self.order_manager.redis_signal_client, 'connection') else "disconnected"
        
        # Vérifier l'état d'exécution des threads
        thread_alive = self.order_manager.processing_thread and self.order_manager.processing_thread.is_alive()
        ...
        diagnostic_info = {
            ...
            "market_data": {
                ...
                "update_time": self.order_manager.last_price_update if hasattr(self.order_manager, 'last_price_update') else None
            },
            "connections": {
                "redis": redis_status,
                "processing_thread": "running" if thread_alive else "stopped",
                "binance": self.order_manager.binance_executor.demo_mode and "demo" or "live"
            },
            "queue_size": self.order_manager.signal_queue.qsize() if hasattr(self.order_manager, 'signal_queue') else None,
    ...
    def create_order(self):
        ...
        try:
            ...
            # Créer l'ordre
            result = self.order_manager.create_manual_order(symbol, side, quantity, price)
    ...
    def resume_trading(self):
        ...
        try:
            ...
            if not symbol and not strategy and hasattr(self.order_manager, 'resume_all'):
    ...
    def diagnostic_binance(self):
        ...
        try:
            ...
            try:
                ...
                signature = self.order_manager.binance_executor._generate_signature(params)
                account_url = f"{self.order_manager.binance_executor.BASE_URL}/api/v3/account"
                params["signature"] = signature
                headers = {"X-MBX-APIKEY": api_key}
                response = requests.get(account_url, params=params, headers=headers, timeout=5)
