...
def circuit_breaker(
    failure_threshold: int = 3, reset_timeout: float = 60.0
) -> Callable:
    """
        ...
        def wrapper(*args, **kwargs):
            with state["lock"]:
                ...
                if state["open"]:
                    ...
                    if time.time() - state["last_failure"] > reset_timeout:
                        logger.info(
                            f"Circuit breaker réinitialisé pour {func.__name__}"
                        )
                        state["open"] = False
                        state["failures"] = 0
                    else:
                        # Circuit toujours ouvert
                        remaining = reset_timeout - (
            ...
            try:
                ...
                with state["lock"]:
                    if state["failures"] > 0:
                        logger.info(
                            f"Circuit breaker réinitialisé après succès pour {func.__name__}"
                        )
                        state["failures"] = 0
...
def validate_price(
    ...
    try:
        ...
        if deviation > max_deviation:
            logger.warning(
                f"❌ Déviation de prix trop importante pour {symbol}: {deviation:.2%} > {max_deviation:.2%}"
            )
            return False
