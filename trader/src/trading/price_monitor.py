...
class PriceMonitor:
    ...
    def __init__(
        self, symbols: list[str], price_update_callback: Callable[[str, float], None]
    ):
        """
    ...
    def _process_price_update(self, _channel: str, data: dict) -> None:
        ...
        try:
            ...
            if old_price:
                ...
                logging.INFO if abs(change_pct) >= 0.5 else logging.DEBUG
            else:
            ...
            logger.error(
                f"❌ Erreur lors du traitement d'une mise à jour de prix: {error_msg}",
                exc_info=True,
            )
            ...
            try:
                ...
                with self.price_lock:
                    for symbol in self.symbols:
                        ...
                        if last_time:
                            # Vérifier si le dernier prix date de plus de 5
                            # minutes
                ...
                for symbol, elapsed in alerts:
                    logger.warning(
                        f"⚠️ Aucune mise à jour de prix pour {symbol} depuis {elapsed:.0f} secondes"
                    )
                ...
                error_msg = str(e) if hasattr(e, "__str__") else repr(e)
                logger.error(
                    f"❌ Erreur dans la vérification des timeouts: {error_msg}",
                    exc_info=True,
                )
