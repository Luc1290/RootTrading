...
class RedisClientPool:
    ...
    def __init__(
        ...
        # Message queue pour chaque PubSub (évite les blocages lors des
        # callbacks)
        ...
        logger.info(
            f"✅ Pool de connexions Redis initialisé pour {host}:{port} (DB: {db})"
        )
    ...
    def _create_connection_pool(self) -> ConnectionPool:
        ...
        try:
            ...
            logger.exception("❌ Erreur de création du pool de connexions Redis")
            raise
        else:
            return pool
    ...
    def decrbyfloat(self, key: str, value: float) -> float:
        ...
        """
        # Redis n'a pas de commande native DECRBYFLOAT, nous utilisons
        # INCRBYFLOAT avec une valeur négative
    ...
    def _retry_operation(self, operation_func, *args, max_retries=3, **kwargs):
        ...
        while retry_count <= max_retries:
            try:
                ...
                # Pause exponentielle avec jitter pour éviter la tempête de
                # reconnexions
                if retry_count <= max_retries:
                    wait_time = (0.1 * (2**retry_count)) + (random.random() * 0.1)
                    logger.warning(
                        f"⚠️ Erreur Redis (tentative {retry_count}/{max_retries}), "
                        f"nouvelle tentative dans {wait_time:.2f}s: {e!s}"
                    )
                    ...
                    try:
                        ...
                        logger.exception("❌ Échec de reconnexion: ")
                else:
                    logger.exception("❌ Échec de l'opération Redis après  tentatives")
            ...
            with self.pubsub_lock:
                for client_id in list(self.pubsub_connections.keys()):
                    ...
                    if channels and callback:
                        ...
                        logger.info(
                            f"✅ Réabonnement aux canaux pour client {client_id}: {channels}"
                        )

        except Exception:
            logger.exception("❌ Échec de la reconnexion Redis")
            raise
    ...
    def _listen_for_messages(
        self, client_id: str, pubsub, message_queue: Queue, stop_event: threading.Event
    ):
        """
        ...
        while not stop_event.is_set():
            try:
                ...
                logger.warning(
                    f"⚠️ Erreur de connexion dans le thread PubSub {client_id} "
                    f"(tentative {retry_count}/{max_retries}): {e!s}"
                )
                ...
                try:
                    ...
                    logger.exception("❌ Échec de reconnexion PubSub pour : ")

            except Exception as e:
                # Si connexion fermée ou arrêt demandé, sortir proprement
                if stop_event.is_set() or "closed file" in str(e).lower():
                    logger.debug(
                        f"Thread d'écoute PubSub {client_id} terminé proprement"
                    )
                    break

                logger.exception("❌ Erreur dans le thread d'écoute PubSub : ")
        ...
        while not stop_event.is_set():
            try:
                ...
                try:
                    ...
                    logger.exception("❌ Erreur dans le callback PubSub pour : ")

                # Indiquer que le traitement est terminé
                message_queue.task_done()

            except Exception:
                logger.exception("❌ Erreur dans le thread de traitement PubSub : ")
    ...
    def subscribe(
        self, channels: str | list[str], callback: Callable[[str, Any], None]
    ) -> str:
        """
        ...
        logger.info(
            f"✅ Abonné aux canaux Redis: {', '.join(channels)} (ID: {client_id})"
        )
        return client_id
        ...
        with self.pubsub_lock:
            ...
            if client_id in self.pubsub_connections:
                try:
                    ...
                    logger.warning(
                        f"⚠️ Erreur lors de la fermeture PubSub {client_id}: {e!s}"
                    )

            # Attendre la fin des threads
            for thread_dict, thread_name in [
                (self.pubsub_threads, "listener"),
                ...
                if client_id in thread_dict:
                    ...
                    if thread.is_alive():
                        ...
                        if thread.is_alive():
                            logger.warning(
                                f"⚠️ Thread {thread_name} pour {client_id} "
    ...
    def set(self, key: str, value: Any, expiration: int | None = None) -> bool:
        """
        ...
        if expiration:
            return self._retry_operation(
                self.redis.setex, key, expiration, formatted_value
            )
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        with self.pubsub_lock:
            ...
            metrics["active_channels"] = sum(
                len(channels) for channels in self.pubsub_channels.values()
            )
    ...
    def __getattr__(self, name):
        ...
        if callable(method):
            ...
            def wrapped_method(*args, **kwargs):
                ...
                formatted_args = []
                for arg in args:
                    if isinstance(arg, dict | list | tuple) and name not in (
                        "hmset",
                        "hset",
                    ):
                ...
                for key, value in kwargs.items():
                    if key == "mapping" and name in ("hmset", "hset"):
                        ...
                        formatted_mapping = {
                            k: self.client._format_value(v) for k, v in value.items()
                        }
