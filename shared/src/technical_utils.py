...
def to_numpy_array(data: Union[List[float], np.ndarray, pd.Series], 
...
def validate_and_align_arrays(*arrays: Union[List[float], np.ndarray, pd.Series],
                             min_length: int = 1,
                             allow_nan: bool = False,
                             alignment: str = 'right') -> Tuple[np.ndarray, ...]:
    """
    ...
    if alignment == 'right':
        ...
        raise ValueError(f"Invalid alignment: {alignment}. Must be 'right', 'left', or 'center'")
    ...
    if max_original - target_length > max_original * 0.1:  # More than 10% truncated
        logger.info(f"Array alignment: {original_lengths} -> {target_length} "
...
def calculate_returns(prices: Union[List[float], np.ndarray], 
                     periods: int = 1,
    ...
    if method == 'simple':
        ...
        returns = (prices_array[periods:] - prices_array[:-periods]) / prices_array[:-periods]
...
def calculate_rolling_stats(data: Union[List[float], np.ndarray],
    ...
    if 'mean' in stats:
        results['mean'] = np.convolve(data_array, np.ones(window)/window, mode='valid')
    
    if 'std' in stats:
        results['std'] = np.array([np.std(data_array[i:i+window]) 
                                  for i in range(len(data_array) - window + 1)])
    
    if 'min' in stats:
        results['min'] = np.array([np.min(data_array[i:i+window]) 
                                  for i in range(len(data_array) - window + 1)])
    
    if 'max' in stats:
        results['max'] = np.array([np.max(data_array[i:i+window]) 
                                  for i in range(len(data_array) - window + 1)])
    
    if 'median' in stats:
        results['median'] = np.array([np.median(data_array[i:i+window]) 
                                     for i in range(len(data_array) - window + 1)])
    
    return results


def detect_outliers(data: Union[List[float], np.ndarray],
                   method: str = 'iqr',
...
def smooth_data(data: Union[List[float], np.ndarray],
               method: str = 'sma',
               window: int = 5,
               **kwargs) -> np.ndarray:
    """
...
def calculate_true_range(highs: Union[List[float], np.ndarray],
                        lows: Union[List[float], np.ndarray],
                        closes: Union[List[float], np.ndarray]) -> np.ndarray:
    """
...
def handle_missing_data(data: Union[List[float], np.ndarray],
                       method: str = 'forward_fill',
                       max_consecutive: int = 5) -> np.ndarray:
    """
