        ...
        with self.lock:
            ...
            for conn in available_connections:
                if conn.check_health():
                    ...
                    logger.info(
                        f"Fermeture d'une connexion inactive (utilisée {conn.usage_count} fois)"
                    )
                    conn.close()
                    self.connection_count -= 1
    ...
    def getconn(self, timeout: float = 30.0) -> ConnectionWrapper:
        ...
        try:
            ...
            if not conn.check_health():
                logger.info(
                    "Connexion invalide récupérée du pool, création d'une nouvelle"
                )
                conn.close()
                self.connection_count -= 1
                conn = self._create_connection()
            ...
            with self.lock:
                if self.connection_count < self.max_connections:
                    try:
                        ...
                        logger.exception("Impossible de créer une nouvelle connexion: ")
                        raise
                    else:
                        return conn
...
class DBConnectionPool:
    ...
    def __init__(self):
        ...
        self.connection_pool = AdvancedConnectionPool(
            DB_MIN_CONNECTIONS, DB_MAX_CONNECTIONS, get_db_url()
        )
        ...
        logger.info(
            f"✅ Pool de connexions initialisé ({DB_MIN_CONNECTIONS}-{DB_MAX_CONNECTIONS})"
        )
            ...
            while True:
                try:
                    ...
                    if db_stats["slow_queries"]:
                        logger.warning(
                            "⚠️ Top requêtes lentes: "
                            + ", ".join(
                                [
                                    f"{q['type']} ({q['duration']:.3f}s)"
                                    for q in db_stats["slow_queries"][:3]
                                ]
                            )
                        )
                    ...
                    logger.exception("Erreur dans le thread de surveillance du pool: ")
    ...
    def get_connection(
        ...
        while attempt <= max_retries:
            try:
                ...
                logger.exception("❌ Erreur lors de l'obtention d'une connexion")
    ...
    def get_diagnostics(self) -> dict[str, Any]:
        ...
        return {"pool": pool_stats, "metrics": db_stats}
...
def real_dict_cursor(auto_transaction=False):
    ...
    """
    db = DBContextManager(
        auto_transaction=auto_transaction, cursor_factory=RealDictCursor
    )
    try:
        cursor = db.__enter__()
        yield cursor
...
def fetch_one(query, params=None, dict_result=True):
    ...
    with DBContextManager(
        auto_transaction=False, cursor_factory=cursor_factory
    ) as cursor:
