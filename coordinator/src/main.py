...
class CoordinatorService:
    ...
    def __init__(self, trader_api_url=None, portfolio_api_url=None, port=5003):
        ...
        """
        self.trader_api_url = trader_api_url or os.getenv(
            "TRADER_API_URL", "http://trader:5002"
        )
        self.portfolio_api_url = portfolio_api_url or os.getenv(
    ...
    def start(self):
        ...
        self.running = True
        logger.info(
            "🚀 Démarrage du service Coordinator RootTrading (version simplifiée)..."
        )
        ...
        try:
            ...
            while portfolio_health_checks < 5:  # Essayer 5 fois maximum
                ...
                logger.warning(
                    f"Tentative {portfolio_health_checks}/5: Service Portfolio non disponible, nouvelle tentative dans 5 secondes..."
                )
                time.sleep(5)
            ...
            if portfolio_health_checks >= 5:
                ...
                while not self.check_portfolio_health():
                    logger.info(
                        "⏳ En attente du service Portfolio... (vérification toutes les 30s)"
                    )
                    time.sleep(30)
            ...
            self.redis_client.subscribe(
                "roottrading:signals:filtered", self.coordinator.process_signal
            )
