                ...
                for row in rows:
                    ...
                    if isinstance(row, dict):
                        ...
                        data_point = {
                            ...
                            "rsi_21": (
                                float(row["rsi_21"])
                                if row["rsi_21"] is not None
                                else None
                            ),
                            "ema_7": (
                                float(row["ema_7"])
                                if row["ema_7"] is not None
                                else None
                            ),
                            "ema_12": (
                                float(row["ema_12"])
                            ...
                            "williams_r": (
                                ...
                                if row["williams_r"] is not None
                                else None
                            ),
                            "cci_20": (
                                float(row["cci_20"])
                                if row["cci_20"] is not None
                                else None
                            ),
                            # Momentum
                            "momentum_10": (
                                float(row["momentum_10"])
                                if row["momentum_10"] is not None
                                else None
                            ),
                            "roc_10": (
                                float(row["roc_10"])
                                if row["roc_10"] is not None
                                else None
                            ),
                            "roc_20": (
                                float(row["roc_20"])
                                if row["roc_20"] is not None
                                else None
                            ),
                            # Volume
                            "obv": (
                            ...
                            "vwap_10": (
                                float(row["vwap_10"])
                                if row["vwap_10"] is not None
                                else None
                            ),
                            "vwap_quote_10": (
                                float(row["vwap_quote_10"])
                                if row["vwap_quote_10"] is not None
                                else None
                            ),
                            "volume_ratio": (
                            ...
                            "avg_volume_20": (
                                ...
                                if row["avg_volume_20"] is not None
                                else None
                            ),
                            "quote_volume_ratio": (
                                float(row["quote_volume_ratio"])
                                if row["quote_volume_ratio"] is not None
                                else None
                            ),
                            "avg_trade_size": (
                                float(row["avg_trade_size"])
                                if row["avg_trade_size"] is not None
                                else None
                            ),
                            "trade_intensity": (
                                float(row["trade_intensity"])
                                if row["trade_intensity"] is not None
                                else None
                            ),
                            # Régime et contexte
                            "market_regime": (
                            ...
                            "regime_strength": (
                                row["regime_strength"]
                                if row["regime_strength"] is not None
                                else None
                            ),
                            "regime_confidence": (
                            ...
                            "volume_context": (
                                row["volume_context"]
                                if row["volume_context"] is not None
                                else None
                            ),
                            "volume_pattern": (
                                row["volume_pattern"]
                                if row["volume_pattern"] is not None
                                else None
                            ),
                            "pattern_detected": (
                                row["pattern_detected"]
                                if row["pattern_detected"] is not None
                                else None
                            ),
                            "data_quality": (
                                row["data_quality"]
                ...
                return [
                    {
                        ...
                        "strength": row[
                            "confidence"
                        ],  # Utiliser confidence comme strength
    ...
    async def _build_current_candle(self, conn, symbol: str, interval_minutes: int):
        ...
        try:
            ...
            period_start = now.replace(
                minute=period_start_minute, second=0, microsecond=0
            )
            ...
            current_candle = {
                ...
                "rsi_14": (
                    ...
                    if last_row["rsi_14"] is not None
                    else None
                ),
                "rsi_21": (
                    float(last_row["rsi_21"])
                    if last_row["rsi_21"] is not None
                    else None
                ),
                "ema_7": (
                ...
                "ema_12": (
                    ...
                    if last_row["ema_12"] is not None
                    else None
                ),
                "ema_26": (
                    float(last_row["ema_26"])
                    if last_row["ema_26"] is not None
                    else None
                ),
                "ema_50": (
                    float(last_row["ema_50"])
                    if last_row["ema_50"] is not None
                    else None
                ),
                "ema_99": (
                    float(last_row["ema_99"])
                    if last_row["ema_99"] is not None
                    else None
                ),
                "sma_20": (
                    float(last_row["sma_20"])
                    if last_row["sma_20"] is not None
                    else None
                ),
                "sma_50": (
                    float(last_row["sma_50"])
                    if last_row["sma_50"] is not None
                    else None
                ),
                "macd_line": (
                    float(last_row["macd_line"])
                    if last_row["macd_line"] is not None
                    else None
                ),
                "macd_signal": (
                    float(last_row["macd_signal"])
                    if last_row["macd_signal"] is not None
                    else None
                ),
                "macd_histogram": (
                    float(last_row["macd_histogram"])
                    if last_row["macd_histogram"] is not None
                    else None
                ),
                "bb_upper": (
                    float(last_row["bb_upper"])
                    if last_row["bb_upper"] is not None
                    else None
                ),
                "bb_middle": (
                    float(last_row["bb_middle"])
                    if last_row["bb_middle"] is not None
                    else None
                ),
                "bb_lower": (
                    float(last_row["bb_lower"])
                    if last_row["bb_lower"] is not None
                    else None
                ),
                "bb_position": (
                    float(last_row["bb_position"])
                    if last_row["bb_position"] is not None
                    else None
                ),
                "bb_width": (
                    float(last_row["bb_width"])
                    if last_row["bb_width"] is not None
                    else None
                ),
                "atr_14": (
                    float(last_row["atr_14"])
                    if last_row["atr_14"] is not None
                    else None
                ),
                "adx_14": (
                    float(last_row["adx_14"])
                    if last_row["adx_14"] is not None
                    else None
                ),
                "stoch_k": (
                    float(last_row["stoch_k"])
                    if last_row["stoch_k"] is not None
                    else None
                ),
                "stoch_d": (
                    float(last_row["stoch_d"])
                    if last_row["stoch_d"] is not None
                    else None
                ),
                "williams_r": (
                    float(last_row["williams_r"])
                    if last_row["williams_r"] is not None
                    else None
                ),
                "cci_20": (
                    float(last_row["cci_20"])
                    if last_row["cci_20"] is not None
                    else None
                ),
                "momentum_10": (
                    float(last_row["momentum_10"])
                    if last_row["momentum_10"] is not None
                    else None
                ),
                "roc_10": (
                    float(last_row["roc_10"])
                    if last_row["roc_10"] is not None
                    else None
                ),
                "roc_20": (
                    float(last_row["roc_20"])
                    if last_row["roc_20"] is not None
                    else None
                ),
                "obv": float(last_row["obv"]) if last_row["obv"] is not None else None,
                "vwap_10": (
                    float(last_row["vwap_10"])
                    if last_row["vwap_10"] is not None
                    else None
                ),
                "vwap_quote_10": (
                    float(last_row["vwap_quote_10"])
                    if last_row["vwap_quote_10"] is not None
                    else None
                ),
                "volume_ratio": (
                    float(last_row["volume_ratio"])
                    if last_row["volume_ratio"] is not None
                    else None
                ),
                "avg_volume_20": (
                    float(last_row["avg_volume_20"])
                    if last_row["avg_volume_20"] is not None
                    else None
                ),
                "quote_volume_ratio": (
                    float(last_row["quote_volume_ratio"])
                    if last_row["quote_volume_ratio"] is not None
                    else None
                ),
                "avg_trade_size": (
                    float(last_row["avg_trade_size"])
                    if last_row["avg_trade_size"] is not None
                    else None
                ),
                "trade_intensity": (
                    float(last_row["trade_intensity"])
                    if last_row["trade_intensity"] is not None
                    else None
                ),
                "market_regime": (
                    last_row["market_regime"]
                    if last_row["market_regime"] is not None
                    else None
                ),
                "regime_strength": (
                    last_row["regime_strength"]
                    if last_row["regime_strength"] is not None
                    else None
                ),
                "regime_confidence": (
                    float(last_row["regime_confidence"])
                    if last_row["regime_confidence"] is not None
                    else None
                ),
                "volume_context": (
                    last_row["volume_context"]
                    if last_row["volume_context"] is not None
                    else None
                ),
                "volume_pattern": (
                    last_row["volume_pattern"]
                    if last_row["volume_pattern"] is not None
                    else None
                ),
                "pattern_detected": (
                    last_row["pattern_detected"]
                    if last_row["pattern_detected"] is not None
                    else None
                ),
                "data_quality": (
                    last_row["data_quality"]
                    if last_row["data_quality"] is not None
                    else None
                ),
            }
                ...
                return [
                    {
                        ...
                        "profit_loss": (
                            float(row["profit_loss"]) if row["profit_loss"] else None
                        ),
                        "profit_loss_percent": (
                            float(row["profit_loss_percent"])
                            if row["profit_loss_percent"]
                            else None
                        ),
