    ...
    async def get_indicators_chart(
        ...
        for indicator in indicators:
            if indicator == "sma":
                ...
                indicator_data["volume_ratio"] = [c.get("volume_ratio") for c in candles]
                indicator_data["avg_volume_20"] = [c.get("avg_volume_20") for c in candles]
                ...
                indicator_data["vwap_quote_10"] = [c.get("vwap_quote_10") for c in candles]
                indicator_data["quote_volume_ratio"] = [c.get("quote_volume_ratio") for c in candles]
                indicator_data["avg_trade_size"] = [c.get("avg_trade_size") for c in candles]
                indicator_data["trade_intensity"] = [c.get("trade_intensity") for c in candles]
            elif indicator == "atr":
                indicator_data["atr_14"] = [c.get("atr_14") for c in candles]
            elif indicator == "regime":
                indicator_data["market_regime"] = [c.get("market_regime") for c in candles]
                indicator_data["regime_strength"] = [c.get("regime_strength") for c in candles]
                indicator_data["regime_confidence"] = [c.get("regime_confidence") for c in candles]
            elif indicator == "volume_context":
                indicator_data["volume_context"] = [c.get("volume_context") for c in candles]
                indicator_data["volume_pattern"] = [c.get("volume_pattern") for c in candles]
                indicator_data["pattern_detected"] = [c.get("pattern_detected") for c in candles]
                indicator_data["data_quality"] = [c.get("data_quality") for c in candles]
