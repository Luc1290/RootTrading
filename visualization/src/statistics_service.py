...
class StatisticsService:
    ...
    async def get_global_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "avgTradeSize": activity.get("volume_24h", 0)
                / max(activity.get("trades_24h", 1), 1),
    ...
    async def get_strategy_comparison(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for strategy_name in all_available_strategies:
                if strategy_name in strategies_with_data:
                    ...
                    if row["trades_executed"] > 0:
                        win_rate = (
                            row["winning_trades"] / row["trades_executed"]
                        ) * 100
            ...
            active_strategies = [
                ...
                if s.get("total_signals_emitted", 0) > 0
            ]
            profitable_strategies = [
                s for s in active_strategies if s.get("totalPnl", 0) > 0
            ]
            ...
            return {
                ...
                "best_strategy": (
                    individual_strategies[0]["strategy"]
                ...
                "total_signals_emitted": sum(
                    s.get("total_signals_emitted", 0) for s in individual_strategies
                ),
                "total_trades_executed": sum(
                    s.get("trades_executed", 0) for s in individual_strategies
                ),
            }
    ...
    async def _get_cycle_statistics(self) -> Dict[str, Any]:
        ...
        stats = {"active_buy": 0, "completed": 0, "cancelled": 0, "failed": 0}
        ...
        return {
            ...
            "completion_rate": (
                round((stats["completed"] / sum(stats.values()) * 100), 2)
    ...
    async def _get_symbol_signal_accuracy(self, symbol: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "best_strategy": (
                max(strategies, key=lambda x: x["accuracy"])["strategy"]
                if strategies
                else None
            ),
        }
    ...
    async def _get_symbol_cycle_analysis(self, symbol: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "best_day": (
                max(daily_stats, key=lambda x: x["pnl"])["date"]
                if daily_stats
                else None
            ),
            "worst_day": (
                min(daily_stats, key=lambda x: x["pnl"])["date"]
                if daily_stats
                else None
            ),
        }
    ...
    async def _get_symbol_market_metrics(self, symbol: str) -> Dict[str, Any]:
        ...
        if result:
            ...
            if row["avg_price"] and row["avg_price"] > 0:
                price_range_percent = (
                    (row["period_high"] - row["period_low"]) / row["avg_price"]
                ) * 100
