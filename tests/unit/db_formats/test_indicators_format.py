...
class TestIndicatorsFormat:
    ...
    def test_indicators_complete_structure(self, indicators_db_format):
        ...
        for indicator in essential_indicators:
            ...
            assert isinstance(indicators[indicator], (int, float)), f"Type invalide pour {indicator}"
    ...
    def test_indicators_value_ranges(self, indicators_db_format):
        ...
        assert 0 <= indicators['stoch_k'] <= 100, f"Stoch %K hors plage: {indicators['stoch_k']}"
        assert 0 <= indicators['stoch_d'] <= 100, f"Stoch %D hors plage: {indicators['stoch_d']}"
        
        # Williams %R doit être entre -100 et 0
        assert -100 <= indicators['williams_r'] <= 0, f"Williams %R hors plage: {indicators['williams_r']}"
    ...
    def test_macd_components_consistency(self, indicators_db_format):
        ...
        assert abs(macd_histogram - expected_histogram) < 0.001, \
    ...
    def test_indicators_json_serializable(self, indicators_db_format):
        ...
        assert deserialized['indicators']['rsi'] == indicators_db_format['indicators']['rsi']
    ...
    def test_indicators_computation_dependency(self):
        ...
        assert abs(test_data['macd'] - expected_macd) < 1.0, \
    ...
    @pytest.mark.parametrize("indicator_name,min_val,max_val", [
        ("rsi", 0, 100),
        ("stoch_k", 0, 100),
        ("stoch_d", 0, 100),
        ("mfi", 0, 100),
