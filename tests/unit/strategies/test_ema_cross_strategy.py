...
class TestEMACrossStrategy:
    ...
    def test_generate_signal_ema_too_close(self, mock_strategy_data):
        ...
        indicators.update({
            'ema_12': 50000,
            'ema_26': 49999  # Distance < min_separation_pct (0.2%)
        })
        data = mock_strategy_data['data'].copy()
        data['close'] = [49000, 49500, 50000]
    ...
    def test_generate_signal_strong_separation_bonus(self, mock_strategy_data):
        ...
        assert result['side'] == "BUY"
        assert "séparation FORTE" in result['reason'] or "séparation forte" in result['reason']
    ...
    def test_generate_signal_macd_confirmation(self, mock_strategy_data):
        ...
        assert result['side'] == "BUY"
        assert ("MACD PARFAITEMENT aligné" in result['reason'] or 
                "MACD confirme" in result['reason'])
    ...
    def test_generate_signal_low_confidence_rejected(self, mock_strategy_data):
        ...
        assert ("confidence insuffisante" in result['reason'] or 
                "EMA trop proches" in result['reason'] or
                "pas de signal clair" in result['reason'])
    ...
    def test_get_current_values_structure(self, mock_strategy_data):
        ...
        strategy = EMA_Cross_Strategy("BTCUSDC", mock_strategy_data['data'], mock_strategy_data['indicators'])
        values = strategy._get_current_values()
        
        # Vérifier que les clés essentielles sont présentes
        required_keys = [
            'ema_12', 'ema_26', 'ema_50', 'ema_99',
    ...
    @pytest.mark.parametrize("ema_12,ema_26,current_price,ema_50,expected_side", [
    ...
    def test_generate_signal_different_crossovers(self, mock_strategy_data, ema_12, ema_26, current_price, ema_50, expected_side):
