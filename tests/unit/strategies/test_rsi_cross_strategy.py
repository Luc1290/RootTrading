...
class TestRSICrossStrategy:
    ...
    def test_generate_signal_buy_oversold_no_trend_confirmation(self, mock_strategy_data):
    ...
    def test_generate_signal_sell_overbought_no_trend_confirmation(self, mock_strategy_data):
    ...
    def test_get_current_values_structure(self, mock_strategy_data):
        ...
        strategy = RSI_Cross_Strategy("BTCUSDC", mock_strategy_data['data'], mock_strategy_data['indicators'])
        values = strategy._get_current_values()
        
        # Vérifier que les clés essentielles sont présentes
        required_keys = [
            'rsi_14', 'rsi_21', 'momentum_score',
            'trend_strength', 'directional_bias', 'confluence_score',
            'volume_quality_score', 'market_regime', 'adx_14'
        ]
        
        for key in required_keys:
            assert key in values
            
    @pytest.mark.parametrize("rsi_value,market_regime,directional_bias,expected_side", [
        (25, 'TRENDING_BULL', 'BULLISH', "BUY"),    # Survente + tendance haussière
        (75, 'TRENDING_BEAR', 'BEARISH', "SELL"),   # Surachat + tendance baissière
        (25, 'TRENDING_BEAR', 'BEARISH', None),     # Survente mais tendance baissière
        (75, 'TRENDING_BULL', 'BULLISH', None),     # Surachat mais tendance haussière
        (50, 'TRENDING_BULL', 'BULLISH', None),     # RSI neutre
    ])
    def test_generate_signal_different_scenarios(self, mock_strategy_data, rsi_value, market_regime, directional_bias, expected_side):
