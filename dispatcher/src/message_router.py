...
class MessageRouter:
    ...
    def __init__(self, redis_client: RedisClient | None = None):
        ...
        for symbol in SYMBOLS:
            for tf in self.timeframes:
                ...
                self.market_data_channel_cache[topic] = (
                    f"{self.redis_prefix}:{self.topic_to_channel['market.data']}:{symbol.lower()}:{tf}"
                )
    ...
    def _determine_priority(self, topic: str) -> str:
        ...
        if topic.startswith(("market.data", "signals", "analyzer.signals")):
            return "high"
    ...
    def _publish_high_priority(
        ...
        try:
            ...
            logger.warning(
                f"❗ Publication haute priorité échouée, message mis en file d'attente: {e!s}"
            )
            raise
                ...
                with self.queue_lock:
                    ...
                    if queue_size > 0:
                        ...
                        try:
                            ...
                            # Incrémenter le compteur de messages haute
                            # priorité traités
                            self.stats["high_priority_routed"] += 1

                            if queue_size > 1:
                                logger.debug(
                                    f"Message haute priorité envoyé depuis la file d'attente. Restants: {queue_size - 1}"
                                )
                        except Exception as e:
                            ...
                            self.high_priority_queue.append((channel, message))
                            logger.warning(
                                f"Échec de publication haute priorité depuis la file d'attente: {e!s}"
                            )
                ...
                with self.queue_lock:
                    ...
                    if queue_size > 0:
                        ...
                        try:
                            ...
                            if queue_size > 1:
                                logger.info(
                                    f"Message envoyé depuis la file d'attente. Restants: {queue_size - 1}"
                                )
                        except Exception as e:
                            # En cas d'échec, remettre le message dans la file
                            # d'attente
    ...
    def _get_redis_channel(self, topic: str) -> str | None:
        """
        ...
        if topic_type == "market" and len(parts) >= 3:
            symbol = parts[2]
            channel = (
                f"{self.redis_prefix}:{self.topic_to_channel['market.data']}:{symbol}"
            )
    ...
    def _transform_message(
        ...
        if any(
            topic.startswith("market.data.") and topic.endswith(f".{tf}")
            for tf in ["1m", "3m", "5m", "15m", "1h", "1d"]
        ):
            ...
            if len(parts) >= 4:
                ...
                for field in required_fields:
                    if field not in transformed:
                        logger.warning(
                            f"Champ requis manquant dans les données de marché: {field}"
                        )
        ...
        elif topic in {"analyzer.signals", "signals"}:
    ...
    def route_message(
        self, topic: str, message: dict[str, Any], priority: str | None = None
    ) -> bool:
        """
        ...
        try:
            ...
            if topic.startswith("market.data") and self.db_persister:
                try:
                    ...
                    logger.exception("❌ Erreur sauvegarde base")
            # =====================================
            ...
            try:
                ...
                if priority == "high":
                    # Pour les messages haute priorité, utiliser une connexion
                    # dédiée
    ...
    def close(self) -> None:
        ...
        if (
            self.high_priority_processor_thread
            and self.high_priority_processor_thread.is_alive()
        ):
        ...
        try:
            with self.queue_lock:
                ...
                if queue_size > 0 or high_priority_size > 0:
                    logger.warning(
                        f"⚠️ Messages non traités lors de la fermeture: {queue_size} normaux, {high_priority_size} haute priorité"
                    )
        except Exception:
            pass
