...
class MessageRouter:
    ...
    def _publish_high_priority(self, channel: str, message: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.warning(
                f"❗ Publication haute priorité échouée, message mis en file d'attente: {str(e)}"
            )
            raise
    ...
    def _process_high_priority_queue(self) -> None:
        ...
        while self.queue_processor_running:
            try:
                ...
                with self.queue_lock:
                    ...
                    if queue_size > 0:
                        ...
                        try:
                            ...
                            self.high_priority_queue.append((channel, message))
                            logger.warning(
                                f"Échec de publication haute priorité depuis la file d'attente: {str(e)}"
                            )
                ...
                logger.error(
                    f"❌ Erreur dans le traitement de la file d'attente haute priorité: {str(e)}"
                )
    ...
    def _process_queue(self) -> None:
        ...
        while self.queue_processor_running:
            try:
                ...
                with self.queue_lock:
                    ...
                    if queue_size > 0:
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Échec de publication depuis la file d'attente: {str(e)}"
                            )
                ...
                logger.error(
                    f"❌ Erreur dans le traitement de la file d'attente: {str(e)}"
                )
    ...
    def _transform_message(self, topic: str, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if any(
            topic.startswith(f"market.data.") and topic.endswith(f".{tf}")
            ...
            if len(parts) >= 4:
                ...
                if has_indicators:
                    logger.warning(
                        f"ATTENTION: Données avec indicateurs reçues du Gateway - architecture corrompue!"
                    )
    ...
    def route_message(
        self, topic: str, message: Dict[str, Any], priority: Optional[str] = None
    ) -> bool:
        """
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"❗ Publication Redis échouée, message mis en file d'attente: {str(e)}"
                )
                return False
