...
class DatabasePersister:
    ...
    def save_market_data(self, topic: str, message: Dict[str, Any]):
        ...
        if not symbol:
            logger.error(
                f"Impossible d'extraire le symbole du topic {topic} ou du message"
            )
            return
            ...
            if self.db_pool is not None:
                async with self.db_pool.acquire() as conn:
                    await conn.execute(
                        ...
                        data["close"],
                        data["volume"],
                        data.get("quote_asset_volume"),
                        data.get("number_of_trades"),
                        data.get("taker_buy_base_asset_volume"),
                        data.get("taker_buy_quote_asset_volume"),
                    )

                    # Log pour les données OHLCV brutes sauvegardées
                    logger.debug(
                        f"✓ Données OHLCV brutes sauvegardées: {data['symbol']} {data['timeframe']} @ {data['close']}"
                    )
