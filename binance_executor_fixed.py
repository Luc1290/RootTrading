...
class BinanceExecutor:
    ...
    def execute_order(self, order: TradeOrder) -> TradeExecution:
        ...
        try:
            ...
            info = self.symbol_info.get(order.symbol, {})
            step_size = info.get("step_size", 0.0001)
            tick_size = info.get("tick_size", 0.01)

            quantity = round(order.quantity / step_size) * step_size
            quantity_str = f"{quantity:.8f}".rstrip("0").rstrip(".")

            price_str = None
            if order.price:
                price = round(order.price / tick_size) * tick_size
            ...
            response = self.session.post(order_url, data=query_string, headers=headers)
            ...
            execution = TradeExecution(
                ...
                price=float(data['price']) if float(data['price']) > 0 else float(data['cummulativeQuoteQty']) / float(data['executedQty']),
